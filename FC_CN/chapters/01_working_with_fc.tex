...
\textit{（原书 Figure 1：Fusion Compiler Physical Synthesis Flow）}\\[0.3em]
\small 输入侧：RTL/netlist、Design（NDM）、Ref.\ lib（NDM）、Tech.\ data、TLU+、UPF、可选 DEF/Tcl、可选 SDC。\\
步骤概览：设置库 $\rightarrow$ 读取 RTL $\rightarrow$ 处理设计不匹配与 black box $\rightarrow$ 施加多电压功耗意图 $\rightarrow$ 施加时序与物理约束 $\rightarrow$ 设置 clock-gating 单元 $\rightarrow$ 设置并插入 DFT $\rightarrow$ 插入多电压单元 $\rightarrow$ 编译设计 $\rightarrow$ 分析与报告 QoR $\rightarrow$ 导出设计。\\
输出侧：SDC、Design（NDM）、Verilog、UPF、DEF/SCANDEF。
\vspace{1.2cm}}}
...
\textit{（原书：Fusion Compiler $\rightarrow$ Formality 流程概览）}
\vspace{1.0cm}}}
...
\textit{（原书 Figure 2：Fusion Compiler Place and Route Flow）}\\[0.3em]
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 3：UPF-Prime / Traditional 与 Golden UPF 流程对比）}
\vspace{1.2cm}}}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.0cm}\textit{（原书 Figure 4：Double-Patterning Example）}\vspace{1.0cm}}}
\caption{双图案化示例}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.0cm}\textit{（原书 Figure 5：Odd-Cycle Violation）}\vspace{1.0cm}}}
\caption{奇环违例}
\label{fig:odd-cycle}
\end{figure}
...
\begin{figure}[htbp]
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.0cm}\textit{（原书 Figure 6：No Odd-Cycle Violation）}\vspace{1.0cm}}}
\caption{无奇环违例}
