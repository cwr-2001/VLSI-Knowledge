...
% 插图开关：true=嵌入 figures/；false=draft 占位（不读入图片，编译更快）
\newif\ifincludefigures
\includefiguresfalse
\usepackage{graphicx}
% 限制插图最大高度，避免单图超页；各处 width=… 仍可单独指定
\setkeys{Gin}{height=0.72\textheight,keepaspectratio}
\ifincludefigures\else
  \setkeys{Gin}{draft,height=0.42\textheight,keepaspectratio}%
\fi
\usepackage{booktabs}
...
\hypersetup{
  ...
  pdftitle={Fusion Compiler User Guide 中文学习译本},
  pdfauthor={Unofficial Study Translation}
}

\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small Fusion Compiler UG V-2023.12-SP3}
...
\begin{center}
\small
原文版权归 Synopsys, Inc.\ 所有（Version V-2023.12-SP3, May 2024）。\\
