# -*- coding: utf-8 -*-
"""Build 10_delay_calculation.tex via Path.write_text."""
...
下列命令调用库缩放与精确匹配：\cmd{set\_operating\_conditions}、\cmd{set\_voltage}、\cmd{set\_temperature}、\cmd{set\_rail\_voltage}。更多信息见 SolvNetPlus 000024652。
...
\begin{seeAlsoBox}
\cmd{set\_timing\_derate} 详见第~\ref{chap:opcond}~章「延时降额」。
\end{seeAlsoBox}
...
print(f"10_delay_calculation.tex: {len(text)} bytes, {len(text.splitlines())} lines, CJK={cjk}")
