# -*- coding: utf-8 -*-
"""Generate 12_case_mode.tex with proper UTF-8."""
from pathlib import Path

text = r"""% 第 12 章 Case and Mode Analysis
...
Case 分析使用端口或 pin 上的逻辑常量或逻辑跳变对时序分析进行约束，从而限制信号在设计中的传播。将 case 分析设为逻辑常量时，常量会向前传播，且常量所传播路径上的路径会被禁用。Case 分析不会向后传播。
...
若单元输入上存在逻辑常量与 case 分析，且逻辑常量被设为该单元的控制逻辑值，则从输出传播逻辑常量；否则传播 case 分析。
...
\end{lstlisting}
        逻辑值 \texttt{static} 表示常量 0 或 1 且无跳变，使所连 net 不能作为串扰 aggressor，也不会作为 victim 经历延时变化。
  \item \textbf{上升或下降跳变}\\
        将 RESET 端口设为上升跳变：
\begin{lstlisting}
...
可通过 case 分析使能或禁用 Liberty 模型中的条件弧。Liberty 是描述无对应门级网表的大块静态时序特性的建模语言。若弧有关联条件，PrimeTime 会评估该条件；设计不满足弧条件时，PrimeTime 禁用该条件弧。注意：条件测试不能使已被 case 分析禁用的弧重新使能。
...
PrimeTime 执行常量传播时，会在常量传播域边界禁用时序弧。要追踪因常量传播导致的弧禁用：
\begin{enumerate}
...
库单元与时序模型可定义工作 mode，例如 RAM 的读/写 mode。每个 mode 关联一组 PrimeTime 在该 mode 激活时分析的时序弧；非激活 mode 的时序弧不分析。未设置任何 mode 时，所有 mode 均激活，所有时序弧均分析。
...
Mode 组织成组。每组有两种可能状态：所有 mode 使能（默认），或仅一个 mode 使能、其余禁用。通常只有一个 mode 组。
...
设计 mode 是设计级用户定义 mode（如芯片的正常/测试 mode）。在 PrimeTime 中创建设计 mode 并映射到单元实例的单元 mode 或设计路径；设置设计 mode 时，关联单元实例设为映射的单元 mode，或关联路径使能、其他设计 mode 的路径设为 false path。
...
out = Path(__file__).resolve().parent.parent / "chapters" / "12_case_mode.tex"
out.write_text(text, encoding="utf-8")
print(f"Wrote {out} ({len(text)} bytes, {text.count(chr(0x4e00)+chr(0x4e8c))} CJK range chars approx)")
# count CJK
import re
cjk = len(re.findall(r'[\u4e00-\u9fff]', text))
print(f"CJK chars: {cjk}, lines: {len(text.splitlines())}")
