# -*- coding: utf-8 -*-
"""Generate 17_advanced_analysis.tex with proper UTF-8 encoding."""
from pathlib import Path

OUT = Path(__file__).resolve().parent.parent / "chapters" / "17_advanced_analysis.tex"

chunks = []

chunks.append(r"""% 第 17 章 Advanced Analysis Techniques
...
# Time borrowing + Advanced latch - continued in part 2
chunks.append(r"""% ============================================================
...
若数据在锁存器透明期间（opening 与 closing 之间）到达，PrimeTime 将下一级建模为从锁存器数据引脚而非时钟引脚启动；无时序违例，slack 视为零。该级借用的时间成为下一级出发时间（受“最大借用时间调整”约束）。数据在 closing 之后到达则为违例。
