# -*- coding: utf-8 -*-
"""Generate 14_multivoltage.tex — Chapter 14 Multivoltage Design Flow."""
from pathlib import Path

OUT = Path(r"d:\IC Design\VLSI\PT_CN\chapters\14_multivoltage.tex")
PARTS = []

PARTS.append(r"""% 第 14 章 Multivoltage Design Flow
...
\begin{itemize}
  \item PrimeTime 计算电压正确的延时与 slew
  \item PrimeTime SI 计算电压正确的串扰延时与噪声效应
...
\begin{itemize}
  ...
  \item 若 PG-pin 库中单元缺少电源或地引脚，PrimeTime 在 link 时移除这些单元，不报告其时序或功耗信息。
...
典型 PrimeTime 多电压时序分析命令序列（UPF 相关加粗）：
\begin{lstlisting}
read_lib l1.lib
read_verilog d1.v
...
\cmd{set\_voltage} 在供电网或单元 PG 引脚上设置电压。\opt{-min} 指定最小延时分析的电压（OCV 角）。\opt{-cell} 与 \opt{-pg\_pin\_name} 在单元级标注 IR drop，须同时使用，需要 PrimeTime SI 许可证。
...
多电压设计中时序路径可跨电源域。当各域可在多种供电电压下工作时，传统分析无法准确分析电压组合对每条路径的影响：每域单一电压则组合无界；每域 min/max（OCV）电压则过于悲观，因同域内单元不能同时工作于不同电压。
...
RedHawk-SC 需要 PrimeTime 的 STA 数据以执行 IR drop、时钟抖动与动态压降分析。
