...
\hline
\tblhead{工作条件} & \tblhead{说明} \\
\hline
工艺降额因子（Process derating factor） & 该值与制造工艺变化所引起的器件参数缩放有关。工艺数值小于标称值通常会产生较小的延时。 \\
...
若逻辑库包含缩放单元信息，则可为特定工作条件包含精确的延时表或系数。对于延时不随工作条件线性缩放的库单元，此方法可获得很高的精度。更多信息见 Library Compiler 与 Design Compiler 文档。
...
\small
\begin{tabular}{|L{1.8cm}|L{1.2cm}|L{3.2cm}|L{3.2cm}|L{3.2cm}|}
\hline
\tblhead{分析模式} & \tblhead{时序检查} & \tblhead{Launch 时钟路径} & \tblhead{数据路径} & \tblhead{Capture 时钟路径} \\
\hline
单一工作条件 & Setup & 晚时钟，时钟路径采用最大延时，单一工作条件（无降额） & 采用最大延时，单一工作条件（无降额） & 早时钟，时钟路径采用最小延时，单一工作条件（无降额） \\
单一工作条件 & Hold & 早时钟，时钟路径采用最小延时，单一工作条件（无降额） & 采用最小延时，单一工作条件（无降额） & 晚时钟，时钟路径采用最大延时，单一工作条件（无降额） \\
\hline
\end{tabular}
\end{table}
...
\small
\begin{tabular}{|L{2.8cm}|L{4.5cm}|L{2cm}|L{2cm}|}
\hline
\tblhead{分析模式} & \tblhead{基于工作条件的延时} & \tblhead{一个 SDF} & \tblhead{两个 SDF} \\
\hline
...
PrimeTime 按下式检查 hold 违例：
\[
\text{clockpath1} + \text{datapathmin} - \text{clockpath2} - \text{hold} \geq 0
\]
其中 clockpath1 $=0.5+0.3=0.8$，datapathmin $=1.6$，clockpath2 $=0.5+0.35=0.85$，hold $=0.1$。由于 $0.8 + 1.6 - 0.85 - 0.1 = 1.45 > 0$，因此不存在 hold 违例。
...
此模式下，给定路径的最大延时按最坏情况的 100\% 计算，最小延时按最坏情况的 80\% 计算。等价的 setup 约束为：
\[
\text{clockpath1} + \text{datapathmax} - \text{clockpath2} + \text{setup} \leq \text{clockperiod}
\]
...
\textbf{示例 1}：在最坏商业工作条件 WCCOM 的基础上，执行下偏 20\% 的 OCV 时序分析：
\begin{lstlisting}
...
\textbf{示例 2}：在预定义工作条件 WCCOM\_scaled 与 WCCOM 之间执行 OCV 时序分析：
\begin{lstlisting}
pt_shell> set_operating_conditions -analysis_type \
...
必须指定 \opt{-early} 或 \opt{-late}，以选择对最短路径延时或最长路径延时应用降额。早、晚降额必须分别用两条命令设置。降额因子为浮点数；要得到考虑 OCV 效应的更保守分析，应使用小于 1.0 的早降额因子或大于 1.0 的晚降额因子。
...
时钟再汇聚悲观是指：两条不同时钟路径共享一部分物理路径，而分析却假定该共享段在一条路径上取最小延时、在另一条路径上取最大延时。当 launch 与 capture 时钟路径采用不同延时值时会出现这种情况，在 OCV 分析中尤为常见。对此悲观量进行自动修正称为时钟再汇聚悲观移除（CRPR）。
...
\cmd{report\_crpr} 报告两个寄存器时钟 pin 或端口之间的 CRP 计算，包括静态和动态条件下计算的时间值，以及其中实际用于悲观移除的值。需指定 launch/capture pin、时钟与检查类型（setup/hold）：
\begin{lstlisting}
...
\cmd{report\_crpr} 还会报告公共节点位置、launch 与 capture 边沿类型（上升或下降）、公共点处四种到达时间（早/晚、上升/下降）、计算得到的上升与下降 CRP，以及实际用于开边沿和关边沿悲观移除的值。
