...
为确保 \cmd{report\_timing} 命令在路径追踪时，对于每一组具有相同时序 sense 的并行单元弧只使用其中的最差弧，将变量 \cmd{timing\_report\_use\_worst\_parallel\_cell\_arc} 设为 \texttt{true}。路径追踪行为如下所示。

\figplaceholder{Figure 200: Worst arc used for path tracing}{路径追踪使用的最差弧}{fig:adv-worst-arc}

仅返回具有给定上升/下降边沿序列，并在逻辑传播过程中使用最差并行弧的路径。最差弧在最小延迟追踪中是最快弧，在最大延迟追踪中是最慢弧。边沿方向序列相同、但使用其他时序弧组合的次关键路径不会返回。不过，不同时序 sense 的弧仍视为彼此不同，因此，对于非单调门，具有不同上升/下降边沿序列的路径仍会返回。
...
该特性在指定 \cmd{-nworst} 大于 1 时同时影响 \cmd{report\_timing} 与 \cmd{get\_timing\_paths}。它只影响具有相同时序 sense 的并行弧。若并行弧具有不同 sense（例如 XOR 门上的 \texttt{positive\_unate} 与 \texttt{negative\_unate}），它们仍被视为不同的弧。该特性只改变报告阶段的路径追踪行为，不影响 \cmd{update\_timing}；因此可随时更改变量值，无需付出重新更新时序的开销。
...
若数据在锁存器透明期间（opening edge 与 closing edge 之间）到达，PrimeTime 将下一级建模为从锁存器数据引脚而非时钟引脚启动；此时没有时序违例，该路径段的 slack 视为零。该级借用的时间成为下一级的启动时间，但仍受“最大借用时间调整”约束。若数据在 closing edge 之后到达，则构成违例。
...
若 opening edge 存在 uncertainty 与 CRPR，端点借用量与给予下一级起点的时间可能不同。确定给予时间时，PrimeTime 会扣除 uncertainty 与 CRPR 调整，使透明模式下的下一级恰在数据到达锁存器数据引脚时启动（图 218）。禁用 \cmd{timing\_early\_launch\_at\_borrowing\_latches} 时，由于工具有意采用较晚的时钟 latency 来启动下一级，数据到达时间与启动时间并不完全相同；启用 CRPR 时建议采用这一模式。在此模式下，不对给予下一级起点的时间应用 CRPR 调整。
