...
\begin{description}[style=nextline, leftmargin=2.5cm, labelwidth=2.2cm, font=\normalfont]
  ...
  \item[AOCV（高级片上变异）（\textit{AOCV}）] Advanced on-chip variation（AOCV）是 Synopsys 技术，根据所分析路径的位置与逻辑深度减少不必要的悲观性。
  ...
  \item[时钟重汇聚悲观性（\textit{clock reconvergence pessimism}）] 静态时序分析的不准确性：两条不同时钟路径部分共享同一段物理路径，分析工具对一条路径用共享段最小延时、对另一条用最大延时。纠正该不准确称为时钟重汇聚悲观性消除（CRPR）。
  ...
  \item[保持约束（\textit{hold constraint}）] 规定数据在锁入器件的时钟边沿之后必须在器件输入保持稳定多长时间的时序约束。相对于时钟对时序路径施加最小延时。
  ...
  \item[发射（\textit{launch}）] 在路径起点用时钟将数据从锁存器或触发器时钟输出的过程，释放的数据由路径端点另一时钟边沿捕获。
  ...
  \item[后版图（\textit{postlayout}）] 版图之后；可获得详细寄生数据用于设计反标的阶段。
  ...
  \item[建立约束（\textit{setup constraint}）] 规定数据在锁入器件的时钟边沿之前必须在器件输入可用多长时间的时序约束。相对于时钟对时序路径施加最大延时。
  ...
  \item[透明（\textit{transparent}）] 电平敏感锁存器门控输入被断言时的状态。此期间信号从输入传到输出，器件如同 buffer。
