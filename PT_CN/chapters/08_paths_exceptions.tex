...
\hline
\tblhead{时序例外} & \tblhead{命令} & \tblhead{说明} \\
\hline
...
新 hold 关系可能不是设计所需的关系。若 FF4 无需在第一个时钟边沿之后继续保持数据，须指定另一时序例外。虽可用 \cmd{set\_min\_delay} 指定显式 hold 时间，但更好的做法是再使用一条 \cmd{set\_multicycle\_path}，将 hold 关系的捕获边沿向较早时间回移一个时钟周期：
...
Figure 85 展示用两条 \cmd{set\_multicycle\_path} 命令正确设置的 setup 与 hold 关系。第二条命令将 hold 关系的捕获边沿向较早时间回移一个时钟周期，即从虚线箭头所示位置移到实线箭头所示位置。
...
\begin{itemize}
  ...
  \item \opt{-hold} 整数值指定相对于默认位置（相对于有效 setup 关系）将捕获边沿向较早时间回移的时钟周期数，默认为 0
\end{itemize}
...
\opt{-setup 5} 使 setup 关系跨越五个时钟周期（time=0 到 time=50），并隐式将 hold 关系改为位于 time=40 的前一捕获边沿。要将 hold 捕获边沿移回 time=0，须用 \opt{-hold 4} 将捕获边沿向较早时间回移四个时钟周期。
...
对所有时序路径，正 margin 使检查更严格；负 margin 使检查更宽松。Path margin 设置是一种时序例外，类似于 \cmd{set\_max\_delay} 或 \cmd{set\_multicycle\_path} 设置的例外；指定的 margin 会叠加到同一路径上适用的 minimum-delay、maximum-delay 和 multicycle path 例外之上。
...
例如，下列冲突例外中，maximum-delay 例外的优先级高于 multicycle path 例外：
\begin{lstlisting}
pt_shell> set_multicycle_path -through buf1/Z -setup 2
pt_shell> set_max_delay -through buf1/Z 1
\end{lstlisting}

\cmd{report\_timing -exceptions dominant} 显示 dominant 例外；\cmd{report\_timing -exceptions overridden} 显示被覆盖且未影响时序计算的例外；\cmd{report\_timing -exceptions all} 显示两者。

\cmd{get\_timing\_paths} 可创建时序路径集合并传递给 \cmd{report\_timing}。时序路径属性（\texttt{dominant\_exception}、\texttt{startpoint\_unconstrained\_reason}、\texttt{endpoint\_unconstrained\_reason}）显示路径未约束原因，如 \texttt{false\_path}、\texttt{min\_delay}/\texttt{max\_delay}、\texttt{multicycle\_path}、\texttt{no\_capture\_clock}、\texttt{no\_launch\_clock}、\texttt{dangling\_end\_point}、\texttt{dangling\_start\_point}、\texttt{fanin\_of\_disabled}、\texttt{fanout\_of\_disabled} 等。

\cmd{report\_timing -exceptions} 始终显示三类信息：dominant exception、startpoint unconstrained reason 和 endpoint unconstrained reason；某一类没有信息时，对应部分为空。上述属性仅在相应信息存在时才出现在 \texttt{timing\_path} 对象上，因此可用 \cmd{filter\_collection} 的 \texttt{defined(...)} 或 \texttt{undefined(...)} 表达式筛选受例外影响的路径。
