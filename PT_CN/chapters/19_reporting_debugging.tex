% 第 19 章 Reporting and Debugging Analysis Results
...
\cmd{-status\_details} 显示各项时序检查的状态详情；对于 untested 检查，如果工具能够确定原因，报告中还会给出原因。\cmd{-exclude\_untested} 可依据原因对列表进行整理；\cmd{-check\_type}、\cmd{-sort\_by slack} 等选项可用于过滤和排序。
...
\textbf{基于路径的时序分析（PBA）} 通过将关键路径与其他路径隔离来降低悲观性：沿每条关注路径传播边沿，忽略该路径沿途侧弧的 slew，重新计算路径专用的到达时间、slew 和 slack，并重新计算 CRPR、串扰 delta delay 等路径效应。它适用于仅剩少量违例、需要判断违例是否源于到达时间或 slew 的悲观性的情形。
...
\cmd{-pba\_mode path} 的每次重算命令最多可处理 2{,}000{,}000 条路径。\cmd{exhaustive} 模式下，每个端点的路径数可由 \cmd{pba\_exhaustive\_endpoint\_path\_limit} 限制。

达到任一限制时，由 \cmd{pba\_path\_recalculation\_limit\_compatibility}（默认 \texttt{true}）控制行为。设为 \texttt{true} 时，不再获取或重算更多路径：对 path-specific 重算，超出限制的剩余路径被丢弃；对 exhaustive 重算，达到限制的端点不再纳入搜索，但该端点已经找到的路径仍保留。设为 \texttt{false} 时，超出限制的结果用原始的基于图分析时序补齐；这些路径仍保持未重算的基于图时序值。

\subsubsection{Exhaustive PBA 设置}
\subsection*{Exhaustive Path-Based Analysis Settings}

\cmd{pba\_exhaustive\_endpoint\_path\_limit} 默认为 \texttt{infinity}：算法将每个端点的 fanin 锥划分为若干子图，逐步分析并修改这些子图，保证覆盖所有路径并返回最差 PBA slack。若设为整数（如 25000），命令会将每条路径与其他路径隔离后分别分析；达到限制时发出 UITE-480 警告并停止分析。当基于图分析与 PBA 的 slack 已较为接近时，整数限制模式可能更快。
