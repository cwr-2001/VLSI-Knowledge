...
超额线程（overthreading）允许同时线程数在短时间内暂时超过最大核数。默认启用。若计算环境不允许超额线程，通过以下变量禁用：
\begin{lstlisting}
...
\end{noteBox}
        必须先完成上述第 1--5 步，才能在下一步运行
        \cmd{current\_session} 命令。
...
详情见各变量 man page。要移除 host 选项并终止关联进程，使用
\cmd{remove\_host\_options} 命令。例如，移除并停止 \texttt{host1} 与
\texttt{host3}：
\begin{lstlisting}
...
\end{lstlisting}
这会把管理进程中数组 \texttt{x} 与 \texttt{y} 指定的值发送到相应场景的
工作进程；若工作进程尚无这些变量，则创建变量并赋予指定值。
...
remote_execute {
  set pin_slack1 [get_attribute -class pin U1/Z slack];
  set pin_slack2 [get_attribute -class pin U2/Z slack]
}
get_distributed_variables {pin_slack1 pin_slack2}
\end{lstlisting}

该命令返回以场景名为索引的 \texttt{pin\_slack1} 与
\texttt{pin\_slack2} 两个数组；例如，
\texttt{pin\_slack1(scen1)} 是场景 \texttt{scen1} 中
\texttt{U1/Z} 的 slack 标量值。
...
空数据值不同于未定义值；数组条目未定义时，
\opt{-null\_merge\_method} 不适用，合并过程会对其余已有数据值正常执行。

\paragraph{跨场景同步对象属性}
例如，在当前命令焦点的所有场景间同步单元 \texttt{U72} 的
\texttt{dont\_touch} 属性：
\begin{lstlisting}
pt_shell> synchronize_attribute -class cell {U72} dont_touch
\end{lstlisting}
此命令在 DMSA 管理进程中工作，可同步用户定义属性或
\texttt{dont\_touch} 应用属性，但不能用于其他应用属性。
\opt{-merge\_type} 可设为 \texttt{min} 或 \texttt{max}，默认
\texttt{max}：数值属性取各场景最大值，字符串属性取字母数字排序中的最后一个值，
布尔属性取 \texttt{true}。
...
\cmd{report\_analysis\_coverage} 命令报告当前会话中所有活动场景的时序检查覆盖率。时序检查由受约束引脚、相关引脚和约束类型共同定义。时序检查状态报告为：\textbf{Violated}（到达时间不满足时序要求）、\textbf{Met}（满足时序要求）、\textbf{Untested}（时序检查被跳过）。仅当时序检查在所有场景中均未测试时才归入 Untested 类别。若至少一个场景中测试了时序检查，则报告为 Violated 或 Met，即使在某些场景中未测试。这允许在功能模式与测试模式等行为显著不同时进行有意义的报告。
...
\begin{itemize}
  \item 若当前会话命令焦点中的全部场景均异常终止，之后在管理进程发出的分布式命令会失败；必须在当前会话中更改命令焦点后再分析所选场景。
  \item 若当前会话中的全部场景均异常终止，当前会话会被移除；之后的分布式命令会因不存在当前会话而报错。
  \item 严重故障会使资源离线；若没有任何资源保持在线，会话必须终止。
\end{itemize}
...
多场景分析可能产生大量数据。为避免在管理进程用数据过载用户，所有数据写入一组日志文件。管理进程仅显示有限的交互消息。使用 \opt{-verbose} 选项时，\cmd{remote\_execute} 将所有数据发送到管理终端。

关于 DMSA 中可用的消息与日志文件类型，请参阅：
\begin{itemize}
  \item 交互消息
  \item 进度消息
  \item 任务执行状态消息的用户控制
...
\subsubsection{交互消息}
\paragraph*{Interactive Messages}

工作进程执行任务时，向管理进程发送消息以指示其进度。

\subsubsection{进度消息}
\paragraph*{Progress Messages}

工作进程在各分析阶段推进时，会发送置信度消息（confidence messages），向您更新分析进度。例如：
\begin{lstlisting}
...
\paragraph*{User Control of Task Execution Status Messages}

可控制 DMSA 任务执行状态消息的详细程度。将 \cmd{multi\_scenario\_message\_verbosity\_level} 变量设为 \texttt{default} 或 \texttt{low}。设为 \texttt{default} 时显示所有任务执行状态消息，例如：
\begin{lstlisting}
Start of Manager/Worker Task Processing
-----------------------------------------------
...
设为 \texttt{low} 时，仅显示 error、fatal 与 failure 消息；这对需要更多输出控制的 Tcl 脚本可能有用。若 Tcl 脚本更改了该变量，宜在脚本或过程完成后将其改回原值，以免造成混淆。

\subsubsection{错误消息}
\paragraph*{Error Messages}

工作进程上给定场景发生错误时，在管理进程完整报告该错误及该场景名。例如：
\begin{lstlisting}
pt_shell> current_session {scen1 scen2}
pt_shell> remote_execute {cmd1}
...
工作进程上给定场景出现警告时，仅当该警告在该场景中首次出现时，才在管理进程报告。例如：
\begin{lstlisting}
pt_shell> current_session {scen1 scen2}
pt_shell> remote_execute {cmd1}
...
可在 log、output log、command log 或 merged error log 文件中查看工作进程生成的全部信息。多场景分析全部数据（含日志文件）的根工作目录由 \cmd{multi\_scenario\_working\_directory} 变量决定。若未显式设置，则设为启动 PrimeTime 管理进程时的当前目录。您必须对该工作目录具有写权限，且该目录须对管理进程及所有工作进程可访问。

\begin{itemize}
  \item \textbf{输出日志（Output Log）}：每个场景有自己的输出日志，位于场景工作目录中。例如，名为 \texttt{s1} 的场景，日志为 \texttt{multi\_scenario\_working\_directory/s1/out.log}。不能设置或更改输出日志名。某一场景的输出日志包含所有 error、warning 与 information 消息，即您在传统单次分析 PrimeTime 运行的终端窗口中通常看到的全部信息。
  \item \textbf{命令日志（Command Log）}：每个远程进程有自己的命令日志，位于工作目录下的 \texttt{command\_log} 目录。例如，在名为 \texttt{srv1} 的主机上启动且为第 6 个启动的进程，命令日志为 \texttt{multi\_scenario\_working\_directory/command\_log/srv1\_6\_pt\_shell\_command.log}。不能设置或更改远程进程的命令日志名。某一进程的命令日志包含在该进程上执行的全部命令。管理进程命令日志位于 \texttt{\$sh\_launch\_dir/pt\_shell\_command.log}。当 \cmd{sh\_source\_logging} 设为 \texttt{true} 时，source 脚本中的各条命令会写入命令日志。
  \item \textbf{合并错误日志（Merged Error Log）}：多场景分析中可能产生大量 error、warning 与 information 消息。为便于调试，可在管理进程设置合并错误日志变量。例如，下列命令在 \texttt{multi\_scenario\_working\_directory/error.log} 中创建合并错误文件：
\begin{lstlisting}
pt_shell> set_app_var multi_scenario_merged_error_log "error.log"
\end{lstlisting}
        设置该变量后，从特定任务开始到结束的所有错误消息合并在一起；若同一错误消息出现在多个场景中，则仅向合并错误文件打印一次，并列出发生该消息的场景。
\begin{noteBox}
须在发出 \cmd{start\_hosts} 命令之前设置 \cmd{multi\_scenario\_working\_directory} 与 \cmd{multi\_scenario\_merged\_error\_log} 变量。
\end{noteBox}
\end{itemize}

\subsubsection{命令输出重定向}
\paragraph*{Command Output Redirection}

多场景分析支持命令输出重定向。须为每个进程重定向到不同目标文件，否则两个不同进程同时试图控制同一文件时会出错。

\begin{noteBox}
使用输出重定向时务必使用相对路径。不要使用绝对路径；否则所有工作进程尝试写入同一文件，可能导致读/写冲突并使工作进程失败。
\end{noteBox}

\cmd{multi\_scenario\_working\_directory} 变量值与场景名一起确定输出目标文件名。交互重定向示例：先执行 \cmd{set x myfile}，再执行 \cmd{remote\_execute \{report\_timing\} > \$x.out}，输出会定向到当前工作目录的 \texttt{myfile.out}，其中 \texttt{\$x} 仅在管理进程求值。脚本重定向示例：\texttt{report\_timing > rt.log} 在 \texttt{/mypath/s3/rt.log} 与 \texttt{/mypath/s4/rt.log} 中分别生成输出。
...
\caption{DMSA 命令}
\small
\begin{tabular}{|L{2.8cm}|L{3.5cm}|L{6.5cm}|}
\hline
\tblhead{类别} & \tblhead{命令} & \tblhead{说明} \\
\hline
...
关键设置如下：\cmd{distributed\_enable\_analysis} 变量启用 HyperGrid；
\cmd{distributed\_working\_directory} 变量设置工作目录（默认
\texttt{dsta\_working\_dir}）；\cmd{set\_host\_options} 与
\cmd{start\_hosts} 命令配置并启动远程工作进程；\cmd{start\_dsta}
命令开始分布式分析（在管理进程划分设计、在工作进程加载划分数据、在管理进程运行指定分析脚本并与工作进程通信）。包装脚本中 \cmd{start\_dsta}
之后的命令在指定分析脚本完成后作为分布式报告命令执行。
...
恢复已保存的分布式会话时，可先按需设置分布式分析工作目录，再按通常方式恢复：
\begin{lstlisting}
...
对于配置为在同一主机上使用多个 CPU 核的进程（如单核或多线程多核分析），\cmd{report\_host\_usage} 提供当前 PrimeTime 会话的资源使用。单核或多线程多核分析流程的报告输出示例如下：
\begin{lstlisting}
...
在 DMSA 流程中，资源可分布在多台主机上，因此为每个分布式进程提供资源使用数据。DMSA 流程的 \cmd{report\_host\_usage} 输出示例如下：
\begin{lstlisting}
pt_shell> report_host_usage
****************************************
Report : host_usage
...
运行不足 30 秒的辅助命令会组成辅助命令捆绑；仅当捆绑总运行时间达到
30 秒或以上时，才视为显著并显式报告。
...
\begin{lstlisting}
define_custom_flow_segment -start -name drc_max_tran
  # commands in this custom segment
define_custom_flow_segment -end -name drc_max_tran
\end{lstlisting}
