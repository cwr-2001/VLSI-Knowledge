...
\cmd{save\_session} 命令不保存属性。要保存在会话期间应用的属性，请使用 \cmd{write\_script} 命令创建 \cmd{pt\_shell}、\cmd{dc\_shell} 或 \cmd{dctcl} 格式的脚本。恢复 PrimeTime 会话后，可使用该脚本在设计上重新创建这些属性。
...
在 PrimeTime SI 分析中，会包含已反标 aggressor 时序窗口的 aggressor stage。

使用 \opt{-ports} 或 \opt{-endpoints} 选项时，设计不必已经完成时序更新。此时，aggressor 使用零输入引脚转换时间和无限时序窗口。这对于复现首次时序更新期间发生的崩溃很有用。
