...
\cmd{set\_clock\_latency} 为一个或多个 clock、port 或 pin 设置 latency。要表示 source latency 的外部不确定性，即 source latency 的取值范围，请使用 \opt{-early} 与 \opt{-late} 选项。
...
前两条命令指定静态 early/late source latency 分别为 3.0 和 5.0；后两条命令在其上叠加 $-0.5$ 和 $+0.5$ 的动态分量，因此总 early/late source latency 分别为 2.5 和 5.5。

若 CLK 使用 propagated latency，则静态与动态 latency 之和用于 delay 计算。不过，对于 launch 与 capture 使用不同时钟边沿的路径，CRPR 在 clock path 公共点之前只使用静态 latency，不使用动态 latency，并从时序结果中移除由此得到的静态悲观量。

\cmd{report\_timing} 分别报告 clock latency 的静态与动态分量。\cmd{report\_crpr} 报告静态和动态条件下计算的 CRP 时间值，以及实际选用于悲观移除的值。
...
\begin{lstlisting}
set delay_calc_waveform_analysis full_design
set timing_enable_min_pulse_width_waveform_adjustment true
set timing_min_pulse_width_low_pulse_threshold 0.12
set timing_min_pulse_width_high_pulse_threshold 0.88
report_min_pulse_width -path_type short ...
\end{lstlisting}

\cmd{report\_min\_pulse\_width -path\_type} 报告会将非默认阈值引起的路径调整显示为 ``open edge threshold voltage adjustment'' 和 ``close edge threshold voltage adjustment''。
...
\begin{lstlisting}
set delay_calc_waveform_analysis full_design
set timing_enable_voltage_swing true
set timing_voltage_swing_low_pulse_threshold 0.10
set timing_voltage_swing_high_pulse_threshold 0.90
report_min_pulse_width -voltage_swing ...
\end{lstlisting}

\figplaceholder{Figure 43: Voltage Swing Checking Thresholds}{电压摆幅检查阈值}{fig:voltage-swing-thresholds}

Voltage swing 检查作为 minimum pulse width 检查的特殊情况执行，minimum width 设为 0.0，并在指定的电源电压比例处测量宽度。达到或越过阈值的任意宽度脉冲均通过检查；未越过阈值的脉冲报告为违例。对于通过检查的脉冲，正 slack 为波形越过阈值电压处测得的脉冲宽度。
