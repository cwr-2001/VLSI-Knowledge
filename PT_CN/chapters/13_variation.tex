% 第 13 章 Variation
\bichapter{片上变异}{Variation}
\label{chap:var}

片上变异（on-chip variation）分析对最小路径与最大路径采用不同的工作条件。为提高精度，可采用以下变异方法：
\begin{itemize}
  \item 高级片上变异（Advanced On-Chip Variation，AOCV）
  \item 参数化片上变异（Parametric On-Chip Variation，POCV）
\end{itemize}
...
高级片上变异（AOCV）分析根据路径中的门数量与路径跨越的物理距离，对变异较小的路径施加较小的降额（derating），从而降低悲观度。AOCV 比传统 OCV 分析的悲观度更低；传统 OCV 采用固定降额因子，不考虑路径特有的度量。

路径越长、门越多，总变异往往越小，因为门与门之间的随机变异会相互抵消。路径在芯片上跨越的物理距离越大，系统性变异往往越大。AOCV 分析根据路径深度与基于位置的包围盒（bounding box）度量，计算路径特定的 AOCV 降额因子。
...
基于图的 AOCV 分析是在 \cmd{update\_timing} 期间执行的快速、全设计范围分析。可利用全设计范围内降低的降额悲观度，以节省面积并提升性能。

就构造方式而言，基于图的分析会计算深度与距离度量的保守值。执行基于图的 AOCV 时，选择保守的路径深度与基于位置的包围盒值，以界定穿过某单元的最坏情况路径。例如，某单元的深度计数不超过穿过该单元的最小深度路径的深度；否则对最小深度路径的分析可能过于乐观。
...
\cmd{timing\_ocvm\_enable\_distance\_analysis} & 指定高级或参数化片上变异分析是否执行基于距离的分析 \\
\cmd{timing\_ocvm\_precedence\_compatibility} & 控制高级或参数化 OCV 对 OCV 降额的回退 \\
\hline
\end{tabularx}
\end{table}
...
\texttt{table} & $N \times M$ 个降额因子；PrimeTime 在表内数据点之间线性插值，不超出表中极值进行外推 \\
\hline
\end{tabularx}
\end{table}
...
% ============================================================
\section{参数化片上变异（POCV）}
\subsection*{Parametric On-Chip Variation (POCV)}

参数化片上变异（POCV）将实例延时建模为实例特有变量的函数，即以该实例独有的延时变量对实例延时进行参数化。POCV 提供：
\begin{itemize}
  \item 针对随机变异的统计单参数降额
...
\cmd{timing\_pocvm\_corner\_sigma} & 指定 POCV 延时与约束分析所用的 corner sigma \\
\cmd{timing\_pocvm\_enable\_analysis} & 启用 POCV 分析 \\
\cmd{timing\_pocvm\_enable\_extended\_moments} & 使用非对称基于矩的建模数据 \\
\cmd{timing\_pocvm\_max\_transition\_sigma} & 指定 POCV 最大 transition time 分析所用的 corner sigma \\
...
使用 POCV 需要以下输入数据之一：
\begin{itemize}
  \item side file 中的 POCV 单一系数
  \item Liberty Variation Format（LVF）中的 POCV slew-load 表
\end{itemize}

若同时读取两种数据，side file 中的 POCV 单一系数优先级更高，会覆盖库中的 POCV slew-load 表。

\subsubsection{Side File 中的 POCV 单一系数}
...
POCV 数据生成通常比 AOCV 更快：AOCV 需对长单元链进行 Monte Carlo 仿真，而 POCV 仅需单级或少数几级。
...
库中的 POCV 数据与 side file 中的单一系数可同时使用；二者作用于同一库单元时，side file 中的单一系数覆盖 LVF 数据。
...
\cmd{report\_timing -variation} 显示变异时序报告。Incr 列中的 Mean、Sensit、Corner 分别给出各增量延时分布自身的均值、标准差和 corner 值；Corner $=$ Mean $\pm K \times$ Sensit，其中 $K$ 为 \cmd{timing\_pocvm\_report\_sigma}。Path 列给出截至该引脚的累积到达时间分布信息。
...
\begin{noteBox}
同时使用 \opt{-from} 与 \opt{-to} 的 \cmd{report\_timing} 可能比仅使用 \opt{-to} 时具有更少的图合并悲观度；因此，\cmd{report\_timing -to xyz} 的结果可作为 \cmd{report\_timing -from abc -to xyz} 结果的保守边界，但二者不一定相等。
\end{noteBox}
