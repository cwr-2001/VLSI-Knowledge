...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item 对 setup 与 hold 约束进行同时的最小与最大延时分析
    \item 在工艺、电压与温度（PVT）条件存在片上变异（on-chip variation，OCV）时进行分析
...
参数化片上变异 POCV（PrimeTime-ADV） &  & X & X \\
...
采用机器学习的穷尽基于路径分析（PrimeTime-ADV-PLUS） &  & X & X \\
HyperTrace 加速基于路径分析（PrimeTime-ADV-PLUS） &  & X & X \\
同时多电压分析 SMVA（PrimeTime-ADV-PLUS） &  & X & X \\
采用基于矩（moment-based）建模的 POCV（PrimeTime-ADV-PLUS） &  & X & X \\
含 Via Variation 的 POCV（PrimeTime-ADV-PLUS） &  & X & X \\
多输入开关分析——\cmd{lib\_arc} 与高级模式（PrimeTime-ADV-PLUS） &  & X & X \\
采用机器学习的加速功耗回收（PrimeTime-ADV-PLUS） &  & X & X \\
...
\begin{itemize}
  ...
  \begin{itemize}
    \item PrimePower 附加工具分析基于单元设计的全芯片功耗耗散（power dissipation）。支持无向量与基于向量的峰值功耗及平均功耗分析。更多信息见 \textit{PrimePower User Guide}。
  \end{itemize}
...
例如，考虑下图所示的两相、基于锁存器的路径。三个锁存器均为电平敏感，当 G 输入为高电平时门有效。L1 与 L3 由 PH1 控制，L2 由 PH2 控制。上升沿从锁存器输出发射数据，下降沿在锁存器输入捕获数据。本例中假设锁存器 setup 时间与延时时间均为零。
...
若数据在时间 = 10 的 PH2 上升沿之前到达 L2（Arrival a），则下一条从 L2 到 L3 的路径数据由时间 = 10 的 PH2 上升沿发射，其工作方式如同同步触发器。此时不从第二条路径借用时间。L2 到 L3 的该时序要求标记为 Setup~2a。
