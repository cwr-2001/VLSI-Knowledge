...
\begin{tabular}{|l|l|}
\hline
\tblhead{许可证} & \tblhead{由 $N$ 计算的许可证数量} \\
\hline
...
\hline
\tblhead{许可证} & \tblhead{场景 A} & \tblhead{场景 B} & \tblhead{合计} \\
\hline
PrimeTime & 3 & 3 & 6 \\
PrimeTime-SI & 3 & 3 & 6 \\
PrimeTime-ADV & 3 & 3 & 6 \\
...
其中：
\begin{itemize}
  \item 「传播的 DVFS 场景数」是跨电源域传播时序信息时实际发现的 DVFS 场景数量；详情见第~\ref{chap:smva}~章「传播的 DVFS 场景」；
...
在含 SMVA 的 DMSA 运行中，总体 SMVA 派生需求始终取所有场景特定
SMVA 需求的 $\max()$。这对两种分布式许可证模式均成立；最终许可证需求再与由 \opt{-max\_cores} 派生的需求取 $\max()$：
\begin{itemize}
  \item \textbf{基于场景的许可证模式}：先按场景分别计算
  \opt{-max\_cores} 需求，再将各场景需求相加。例如，两个场景各有
  50 个 SMVA 场景且均使用 \opt{-max\_cores 80} 时，对于 PrimeTime 至
  PrimeTime-ADV-PLUS：
  \[
    \max\bigl(\lceil 50/8\rceil,\ \lceil 50/8\rceil,\
    \lceil 80/32\rceil+\lceil 80/32\rceil\bigr)=7.
  \]
  PrimeTime-ELT 与 PrimeTime-APX 不受 SMVA 派生需求影响，分别需要 6 份和 4 份。
  \item \textbf{基于核的许可证模式}：按所有场景的总核数计算
  \opt{-max\_cores} 需求。对同一示例，PrimeTime 至 PrimeTime-ADV-PLUS：
  \[
    \max\bigl(\lceil 50/8\rceil,\ \lceil 50/8\rceil,\
    \lceil(80+80)/32\rceil\bigr)=7.
  \]
  PrimeTime-ELT 与 PrimeTime-APX 分别需要
  $\lceil(80+80)/32\rceil=5$ 份和
  $\lceil(80+80)/64\rceil=3$ 份。
\end{itemize}
