...
使用 ECO > Insert Buffer 或 ECO > Size Cell（\cmd{insert\_buffer} / \cmd{size\_cell}）执行手动 ECO 时，工具会自动在内存中执行增量路径时序更新，因此可立即查看更改对时序的影响。
...
# Display, categorize, and sort paths from multiple sessions in the GUI
...
\end{lstlisting}
...
\cmd{gui\_change\_highlight} 用于操纵全局高亮对象集。有关具体 GUI Tcl 命令的语法与参数，请查阅相应的 man page。
