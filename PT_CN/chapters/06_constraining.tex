...
对端口应用 \cmd{set\_drive} 或 \cmd{set\_driving\_cell} 命令，会使该端口具有 cell delay，其值为外部驱动单元延迟中与负载相关的部分。为避免重复计入该延迟，应估算驱动单元延迟中与负载相关的部分，再从该端口的输入延迟中减去这一数值。

输入延迟应等于从源触发器时钟引脚到驱动单元输出引脚的路径延迟，减去驱动单元延迟中与负载相关的部分。下图展示从 L1 时钟端口到 IN1 端口的外部路径。
...
默认情况下，对 pin 或 port 执行约束检查时，采用 design、pin、port、clock（若该 pin 或 port 位于相应 clock domain 中）或 library 上指定的最严格约束。多个 clock 发射同一路径时也是如此。
...
\cmd{set\_max\_transition} 在指定对象上设置 \texttt{max\_transition} 属性（设计规则约束）。PrimeTime 以各 pin 或 library 的本地 threshold 与 derate 报告 slew 和最大 transition 约束属性。例如，在 design threshold 和 derate 语境下设置的最大 transition，会缩放到 design pin 的 threshold 和 derate；默认启用这种基于 slew threshold 的缩放。要对 \texttt{max\_transition} 值应用全局 derating factor，请设置 \texttt{timing\_max\_transition\_derate} 变量。
...
\textbf{示例 4}：若 library \texttt{lib1} 的 slew threshold 为 10/90，design 上设置的最大 transition 为 0.3\,ns，而 main library 的上升/下降 slew threshold 为 30/70，则该限制换算到 \texttt{lib1} pin 的 threshold 后为 0.6\,ns。此时 \cmd{report\_constraint -max\_transition -all\_violators -significant\_digits 4} 会相应报告 required transition、actual transition 和 slack。

\textbf{示例 5}：若 library \texttt{lib1} 的 slew derating factor 为 0.5、上升/下降 slew threshold 为 30/70，design 上设置的最大 transition 为 0.3\,ns，而 main library 的 slew derate 为 1.0，则换算后的 required transition 同样为 0.6\,ns。
...
也可使用 \cmd{report\_constraint} 的 \opt{-object\_list} 选项，仅报告指定 port 或 pin 的最大 capacitance 与最大 transition 约束计算。
...
查看最小 capacitance 约束评估，使用 \cmd{report\_constraint -min\_capacitance}；查看 port capacitance 限制，使用 \cmd{report\_port -design\_rule}；查看当前 design 的默认 capacitance 设置，使用 \cmd{report\_design}；移除用户指定的最小 capacitance 限制，使用 \cmd{remove\_min\_capacitance}。
...
\cmd{set\_max\_capacitance} 应用 \texttt{max\_capacitance} 属性。Capacitance 约束检查仅适用于 output pin，检查时采用最严格约束。对 \texttt{max\_capacitance} 值应用全局 derating，设置 \texttt{timing\_max\_capacitance\_derate} 变量。

查看最大 capacitance 约束评估，使用 \cmd{report\_constraint -max\_capacitance}；查看 port capacitance 限制，使用 \cmd{report\_port -design\_rule}；查看当前 design 的默认 capacitance 设置，使用 \cmd{report\_design}；移除用户指定的最大 capacitance 限制，使用 \cmd{remove\_max\_capacitance}。

\subsection{约束上升与下降最大电容}
\subsubsection*{Constraining Rise and Fall Maximum Capacitance}

要约束 design 中 clock 的 clock path 和 data path 上 driver pin 输出端的上升与下降最大 capacitance，请使用 \cmd{set\_max\_capacitance}。工具会将 clock 上的最大 capacitance 约束应用于 clock network 中的所有 pin（clock path），以及由该 clock 发射的数据路径中的 pin（该 clock 的 data path）。可进一步用 \opt{-clock\_path}、\opt{-data\_path}、\opt{-rise} 和 \opt{-fall} 限制约束范围。

\figplaceholder{Figure 31: Restrict constraints}{限制约束范围}{fig:restrict-constraints}

若对象为 clock 列表，可用 \opt{-clock\_path}、\opt{-data\_path}、\opt{-rise} 或 \opt{-fall} 进一步限制约束范围。若 pin（或 port）受 design、library、clock（以及 port）上不同最大 capacitance 约束，最严格约束优先。slack 为最严格约束与 pin（或 port）总 capacitance 之差。该特性适用于 generated clock。使用 \cmd{report\_constraint} 报告约束违例。
...
查看最大 fanout 约束评估，使用 \cmd{report\_constraint -max\_fanout}；查看 port 的最大 fanout 限制，使用 \cmd{report\_port -design\_rule}；查看当前 design 的默认最大 fanout 设置，使用 \cmd{report\_design}；撤销用户在 port 或 design 上设置的最大 fanout 限制，使用 \cmd{remove\_max\_fanout}。
...
使用 \opt{-no\_propagate} 限制传播范围，仅将与 ideal network 源电气相连的 net 与 pin 标记为 ideal：
\begin{lstlisting}
pt_shell> set_ideal_network -no_propagate net1
\end{lstlisting}

在此示例中，只有 \texttt{net1}、它的 driver pin 和 load pin 被标记为 ideal，不再继续传播。若不指定 \opt{-no\_propagate}，PrimeTime 会在给定的 pin、port 或 net 列表上设置 ideal 属性，并按前述规则继续传播。
