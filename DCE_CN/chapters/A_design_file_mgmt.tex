...
\begin{itemize}
  \item 高度结构化逻辑（如通常包含大型异或树的检错电路）更适合 structuring。
...
DC Explorer 可共享大型资源（如加法器或乘法器），但资源共享仅当资源属于同一 VHDL process 或 Verilog always 块时才能发生。
...
为获得正确仿真结果：
\begin{itemize}
  \item 在时序 Verilog \texttt{always} 块或 VHDL \texttt{process} 内使用延迟（非阻塞）赋值
  \item 在组合 Verilog \texttt{always} 块或 VHDL \texttt{process} 内使用立即（阻塞）赋值
\end{itemize}
...
\begin{itemize}
  ...
  \item 不要使用 \texttt{buffer} 端口模式。\\
        将端口声明为 \texttt{buffer} 时，该端口必须在整个层次中用作 \texttt{buffer}。
        为简化综合，将端口声明为 \texttt{output}，然后定义驱动输出端口的内部信号。
\end{itemize}
...
\begin{itemize}
  ...
  \item 避免保留字。
  \item 对名称使用名词或名词后跟动词的形式，例如 \texttt{AddrDecode}、\texttt{DataGrant}、\texttt{PCI\_interrupt}。
  \item 添加后缀以澄清名称含义。\\
...
\end{lstlisting}
  ...
  \item 使用组件蕴含（component implication）时要小心。\\
        可用 \texttt{map\_to\_module} 与 \texttt{return\_port\_name} 编译器指令将函数映射到特定实现。
        仿真使用函数内容；综合用门级模块替代函数。使用组件蕴含时，RTL 模型与门级模型可能不同。
