...
DC Explorer 为综合与时序分析提供两种界面：\cmd{de\_shell} command-line interface（shell）和 Design Vision graphical user interface（GUI）。
...
\begin{itemize}
  \item \opt{-checkout}：访问程序默认检出功能以外的 licensed feature
  \item \opt{-wait}：为检出任何额外 license 设置等待时间上限
  \item \opt{-f}：在显示初始 \cmd{de\_shell} 提示符之前执行脚本文件
  \item \opt{-x}：在启动时执行一条 \cmd{de\_shell} 语句
  \item \opt{-no\_init}：指定 \cmd{de\_shell} 不执行任何 \texttt{.synopsys\_dc.setup} startup file\\
        仅当要包含 command log 或其他 script file，以复现先前的 \cmd{de\_shell} session 时，才使用此选项。
  \item \opt{-no\_home\_init}：指定不执行 home directory 中的 \texttt{.synopsys\_dc.setup} startup file
  \item \opt{-no\_local\_init}：指定不执行 local \texttt{.synopsys\_dc.setup} startup file
...
\begin{enumerate}
  ...
  \item 在调用 \cmd{de\_shell} 的窗口中显示 program header 与 \cmd{de\_shell} prompt。program header 列出站点已获得 license 的所有 feature。
\end{enumerate}

\begin{noteBox}
若以路径形式输入 \cmd{de\_shell} 命令，必须指定完整的 absolute path，而不能使用 relative path。
若使用 relative path（如 \texttt{../}），DC Explorer 将无法访问位于 root directory 中的 library。
...
输入命令、选项或文件名时，键入足以唯一标识名称的字符后可按 Tab 键，由 DC Explorer 补全其余字符。
若已键入的字符可对应多个名称，工具会列出符合条件的名称，可用方向键和 Enter 键进行选择。
...
要关闭 GUI 而不退出 DC Explorer，可任选其一：
\begin{itemize}
...
这些文件包含用于初始化参数和变量、声明 design library 等操作的命令。
...
您的主目录 &
包含用于定义 DC Explorer 工作环境 preference 的变量。此文件中的变量覆盖 system-wide setup 文件中的对应变量。 \\
设计专用 \texttt{.synopsys\_dc.setup} &
启动 DC Explorer 的工作目录 &
包含影响该目录中所有设计优化的 project-specific 或 design-specific 变量。要使用此文件，必须从该目录调用 DC Explorer。此文件中定义的变量覆盖 user-defined 和 system-wide setup 文件中的对应变量。 \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}
  \item \textbf{ASCII log}\\
        该 log 在屏幕上显示每次 DC Explorer 运行的输出（例如已处理的命令与 error message），便于快速查看和调试。
        日志末尾的摘要显示有多少消息被重定向到 \texttt{.DE\_log\_snapshot\_date\_process\_id} 目录下各具体日志文件。

  \item \textbf{HTML log file}\\
...
\begin{noteBox}
必须安装 Python 才能生成 HTML log file。下载信息见：\url{https://www.python.org/download}

默认情况下，工具将非关键 warning 和 information message 从 ASCII log 重定向到 HTML log file。
...
Design Vision（GUI）和 DC-Extension license 会按需自动检出。

\begin{noteBox}
使用 multicore processing 时，每两个 core 需要一份 license。要移除 multicore processing 期间额外检出的 license，请使用 \cmd{remove\_license} 命令。
\end{noteBox}
...
要显示哪些 license 已被检出，使用 \cmd{license\_users} 命令。例如：
\begin{lstlisting}
...
默认情况下，每个 feature 仅检出一份 license。license 检出后会一直保留，直到您释放它或退出 \cmd{de\_shell}。
...
\end{lstlisting}
可用 \opt{-keep} 选项指定命令完成后为每个 feature 保留的 license 数量。
若不指定该选项，运行 \cmd{remove\_license} 时会释放每个 feature 的所有 license。
...
DC Explorer 提供 license queuing 功能：当所有 license 均在使用时，应用程序可等待 license 变为可用。
...
DC Explorer 的 multicore technology 支持使用多个 core 来缩短工具 runtime。
在综合期间，该技术可将大型优化任务划分为较小任务，以便在多个 core 上处理。
...
\end{lstlisting}
所有 \cmd{compile\_exploration} 命令选项均支持使用多个 core 进行优化。

启用多核处理后，日志文件会包含类似如下的信息消息：
\begin{lstlisting}
Information: Running optimization using a maximum of 6 cores. (OPT-1500)
\end{lstlisting}
为获得最高效率，\opt{-max\_cores} 设置不应大于实际可用 core 数。

设置 \varname{compile\_adjust\_max\_processes\_used} 变量时，工具按该变量的下列取值运行多核进程：
\begin{itemize}
  \item \texttt{1}：默认情况下，若 \texttt{overcommit\_memory} 参数设为 2，工具会在 multiprocessing 期间调整 core 数。
        若资源不足以并发运行新进程：
        \begin{itemize}
          \item \cmd{compile} 或 \cmd{compile\_ultra} 会调整 core 数并继续执行，而不是终止进程。
                但是，当且仅当至少有一个新进程成功启动时，命令才会继续执行。
...
要报告总体 compile wall clock time，请运行 \cmd{report\_qor} 命令，如下例所示。
该命令报告 \cmd{compile\_exploration} 的 wall clock time。
\begin{lstlisting}
de_shell > report_qor
****************************************
Report : qor
...
衡量 multicore optimization 的 runtime speedup 时，应使用进程的 wall clock time。CPU time 不是衡量 multicore runtime speedup 的正确指标。
...
为缩短 runtime 并生成相同的报告，可使用 parallel command execution 并行运行这些命令。
\begin{itemize}
...
默认情况下，每八个 core 需要一套 DC Explorer license。

\subsection{多核处理}
\subsubsection*{Multicore Processing}

parallel command execution 最多可使用八个 core。一次可并行执行的最大命令数由 \cmd{set\_host\_options} 的 \opt{-max\_cores} 选项决定。
若指定大于八的数字，工具会发出 warning，并以可用 core 数覆盖该设置。
...
要缩短 DC Explorer runtime，可对 \cmd{redirect} 命令使用 \opt{-bg} 选项，在前台运行其他命令的同时，在后台运行命令。
