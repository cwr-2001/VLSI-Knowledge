...
优化设计时，可能需要对不同 hierarchy level 和 component 采用不同的 compile strategy。
...
该设计示例说明如何使用一组常用 \cmd{de\_shell} 命令设置 design constraint，以及如何用 script 实现不同的 compile strategy。
...
\begin{description}[style=nextline,leftmargin=3cm,font=\ttfamily]
  \item[Adder16]
        使用 registered output 以简化 constraint 设置。由于 endpoint 是 register 的 data pin，无需在 output port 上设置 output delay。
  \item[CascadeMod]
        使用 hierarchical compile strategy。该设计的 compile script 在 compile 前于顶层（\texttt{CascadeMod}）设置 constraint。\\
        \texttt{CascadeMod} 设计两次实例化 \texttt{Adder8} 设计。script 对 \texttt{Comparator} module 使用 compile-once-don't-touch 方法。
  \item[Comparator]
  ...
  \item[Multiply8x8]
        说明用于优化设计的基本时序与面积约束。
  \item[Multiply16x16]
        在 compile 前 ungroup DesignWare component。Ungroup hierarchical module 可能有助于获得更好的 synthesis result。
        该 module 的 compile script 在 module 的 primary port 上定义 two-cycle path。
  \item[MuxMod]
        是组合块。该设计的脚本使用虚时钟概念。
  \item[PathSegment]
        在 module 内使用 path segmentation。script 对 module 内的 two-cycle path 使用 \cmd{set\_multicycle\_path}，并用 \cmd{group} 创建新的 hierarchy level。
\end{description}
...
该 setup file 使用 Tcl subset 编写，可供 dctcl command language 使用。
...
若某 module 的 script 包含额外 constraint，或包含不同于 default constraint file 的 constraint value，DC Explorer 将采用 module-specific constraint。
