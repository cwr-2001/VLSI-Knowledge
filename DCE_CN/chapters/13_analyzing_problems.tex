...
在 DC Explorer 中，先运行 \cmd{write\_environment}，再运行 \cmd{compile\_exploration}；在 IC Compiler 中，先运行 \cmd{write\_environment}，再运行 \cmd{place\_opt}。
...
设计不包含下列错误时，称为一致：未连接端口、常量值端口、无输入或输出引脚的 cell、cell 与其 reference 不匹配、multiple-driver net、connection-class violation 或递归层次定义。
...
默认情况下，该命令列出每个 path group 中最长的 max-delay timing path 的完整路径。
DC Explorer 根据控制 endpoint 的 clock 对路径分组；所有未与 clock 关联的路径均属于 default path group。
...
\cmd{create\_qor\_snapshot} 从 timing、design rule、area、power、congestion、clock tree synthesis、routing 等方面度量并报告设计质量，并将 QoR 信息存入一组 snapshot 文件。
...
例如，可针对指定数值范围（如 WNS、logic level 等）生成 \cmd{set\_false\_path} constraint。
该命令不对设计执行额外分析，仅处理已保存在快照报告文件中的报告信息；因此其执行远快于 \cmd{create\_qor\_snapshot}。

可用 \opt{-filters} 选项过滤路径。例如，下列命令报告 WNS 小于 0 的所有路径：
\begin{lstlisting}
...
有时 worst-timing path 从某一 hierarchical block 开始并在另一 hierarchical block 结束；识别这些路径并对其分组可能很困难。
\opt{-hierarchy} 选项启用 top-level view，并自动查找跨越 hierarchy boundary 的潜在 path grouping，以便更高效地分析问题路径。
...
工具为每个 hierarchy 创建一个 path group；若设计包含多个 hierarchy level，则为每一层创建 path group。
...
\subsection*{RTL Cross-Probing Using Commands}

为改善时序、拥塞或 datapath 提取，可在命令行使用 RTL 交叉探测能力检查 RTL 设计，并识别需修改的代码行。

要使用下列 cross-probing 命令，设计必须已 analyze 并 elaborate，或已综合：
\begin{itemize}
...
该命令报告所有路径类型的 logic level，包括 same-clock path、cross-clock path、infeasible path 与 multicycle path。
报告的 logic-level 数量不计 buffer 和 inverter。
