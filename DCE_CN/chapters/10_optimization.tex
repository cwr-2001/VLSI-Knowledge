...
可用 \cmd{compile\_exploration} 获得更好的 QoR，尤其适用于时序约束非常紧的高性能设计。该命令允许在 compile 期间应用尽可能合适的一组 timing-centric variable，以优化 critical delay 并改善 area QoR。
...
命名约定由 \varname{uniquify\_naming\_style} 指定，默认 \texttt{\%s\_\%d}（\texttt{\%s} 为原 subdesign 名或 \opt{-base\_name} 指定的名称；\texttt{\%d} 为形成唯一 subdesign 名的最小整数）。
...
可将 sequential cell 与 instantiated combinational cell 的 mapping 和 optimization 限制到 target library 中用户指定的 library cell 子集。
对 \cmd{define\_libcell\_subset} 使用 \opt{-family\_name} 定义一个或多个子集；仅当库单元满足下列条件时才归入子集：
\begin{itemize}
  \item 必须是 sequential cell 或 instantiated combinational cell
  \item 不能已属于另一子集
  \item 必须具有相同的 functional identification
\end{itemize}

分组后，这些 library cell 在 compile 期间不再用于一般 mapping；DC Explorer 仅使用它们映射由 \cmd{set\_libcell\_subset} 指定的 sequential cell 与 instantiated combinational cell。用 \opt{-object\_list} 指定要优化的 cell 列表；指定 cell 必须尚未 mapping，或已实例化为该子集中的某个 library cell。
...
DC Explorer 可跨多个 mode 与多个 corner 并发分析和优化 multicorner-multimode（MCMM）设计。由于用户界面相似，该功能在 DC Explorer 与 IC Compiler flow 之间提供了易用性和兼容性。
...
\begin{itemize}
  ...
  \item \textbf{Mode} — 由一组 clock、supply voltage、timing constraint 和 library 定义；也可包含 SDF 或 parasitics file 等 annotation data。MCMM 设计可工作于 test、mission、standby 等多种 mode。
...
为 MCMM flow 设置设计时，必须为每个 scenario 指定 TLUPlus file、operating condition 和 SDC constraint。DC Explorer 按 nominal process、voltage 和 temperature（PVT）值将 library 分组；PVT 相同的 library 归入同一集合，并使用各 scenario 的 maximum operating condition 的 PVT 值选择相应集合。
...
\end{lstlisting}
  ...
  \item 用 \cmd{set\_tlu\_plus\_files} 为每个 scenario 指定 TLUPlus file。要启用 temperature scaling，TLUPlus file 必须包含 \texttt{GLOBAL\_TEMPERATURE} 和 \texttt{CRT1}；\texttt{CRT2} 可选。
...
用 \cmd{set\_optimize\_registers} 启用 pipelined-logic retiming。在逻辑综合前用 RTL 描述电路时，寻找最佳 register 位置并将其编码到 HDL 中非常困难且耗时。通过 register retiming，可自动调整 sequential design 中 flip-flop 的位置，使各 pipeline stage 的 delay 尽可能均衡。

\cmd{set\_optimize\_registers} 所用技术对 pipelined datapath block 特别有用：先指定所需 pipeline stage 数量，并在 RTL 中把全部 pipeline register 放在 combinational logic 的输入端或输出端；retiming 期间，工具将这些 register 移入 combinational logic、平衡各 stage 的 delay，并根据给定 clock period 获得最佳 timing 和 area 结果。
...
\subsection{保留 Subdesign}
\subsubsection*{Preserving Subdesigns}

要在 optimization 期间保留 subdesign，使用 \cmd{set\_dont\_touch}。该命令在当前设计的 cell、net、reference 和 design 上设置 \texttt{dont\_touch} 属性，防止这些对象在 optimization 期间被修改或替换。

\begin{noteBox}
设计中的任何 block abstraction 都会自动标记 \texttt{dont\_touch}；block abstraction 中的 cell 也会标记为 don't touch。
\end{noteBox}

可用 \cmd{set\_dont\_touch} 指定不与设计层次其余部分一起优化的 subdesign。\texttt{dont\_touch} 属性不会阻止或禁用穿过该 design 的 timing。使用该命令时应注意：
\begin{itemize}
  \item 在 hierarchical cell 上设置 \texttt{dont\_touch}，会对该 cell 以下的全部 cell 隐式设置 don't touch。
  \item 在 library cell 上设置该属性，会对该 library cell 的全部 instance 隐式设置 don't touch。
  \item 在 net 上设置该属性，仅对连接到该 net 的 mapped combinational cell 隐式设置 don't touch。若该 net 仅连接 generic logic，optimization 仍可能移除该 net。
  \item 在 reference 上设置该属性，会在后续 design optimization 中对使用该 reference 的全部 cell 隐式设置 don't touch。
  \item 在 design 上设置该属性，仅当该 design 作为一个层次级实例化于另一 design 中时才有效；此时该层次以下的全部 cell 都受 \texttt{dont\_touch} 约束。在 top-level design 上设置无效，因为 top-level design 不会实例化在其他 design 中。
  \item 不能用 \cmd{ungroup} 对标有 \texttt{dont\_touch} 的对象执行 ungroup；DC Explorer 的 automatic ungrouping 也不会影响 don't-touch object。
\end{itemize}

\begin{noteBox}
对 synthetic part cell（例如从 HDL description 读入的许多 cell）以及连接有 unmapped cell 的 net，\texttt{dont\_touch} 属性会被忽略。Compile 期间，对于连接到 unmapped cell（generic logic）的 don't-touch net，工具会发出警告。
\end{noteBox}

要移除 \texttt{dont\_touch} 属性，使用 \cmd{remove\_attribute}，或用 \cmd{set\_dont\_touch} 将其设为 \texttt{false}。
...
拥塞优化期间，拥塞区域中的 cell 会扩展以占据更多空间，从而形成较低密度区域。工具生成 cell expansion data，以改善与 IC Compiler 和 IC Compiler II 的 congestion correlation；数据包括每个 expanded cell 的 area、width 和 height。分别使用 \cmd{read\_cell\_expansion} 和 \cmd{write\_cell\_expansion} 读取和写出：
\begin{lstlisting}
