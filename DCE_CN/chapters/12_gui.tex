...
\begin{itemize}
  ...
  \item 分析可视化能力，包括：
  \begin{itemize}
    \item 层次浏览器（hierarchy browser），用于在设计层次中导航并探索设计结构，包括查看面积属性
  ...
  \end{itemize}
  \item 报告能力，可将报告对象与图形视图关联
  \item 集成于 GUI 的命令行界面，支持全部 Design Compiler 工具命令语言（dctcl）命令的脚本
...
\begin{itemize}
  \item Design Vision 主窗口：从 \cmd{de\_shell} 打开 GUI 时自动出现。\\
        默认在 Design Vision 主窗口中显示层次浏览器（逻辑层次视图）与命令控制台（console panel）。
...
\begin{itemize}
  ...
  \item 点击命令行上方的选项卡，查看会话转录（log 视图）或命令历史列表（history 视图）
  \item 复制、编辑或重用命令，并搜索、选择与保存命令或消息
\end{itemize}

可在控制台命令行输入任意 \cmd{de\_shell} 命令。
发出命令后（按 Return，或点击命令行左侧的提示按钮），控制台与 \cmd{de\_shell} 都会回显命令输出，包括处理消息以及任何警告或错误消息。
...
\begin{enumerate}
  ...
  \item 在 Work Directory 框中，输入相对于当前目录的工作目录相对路径名。\\
...
\end{lstlisting}
  \item 点击 \textbf{OK}。\\
        DC Explorer 以简化的 floorplanning 菜单在 IC Compiler 版图窗口中调用 floorplan 探索。
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{物理层次与块抽象}\\
        ...
        物理层次与块抽象以带有块内快速时序模型（quick timing model）的宏形式传输。\\
...
\begin{itemize}
  \item \textbf{综合设计}\\
        不能对未综合（或未映射）的设计运行 IC Compiler II。
...
\begin{enumerate}
  ...
  \item 在 \cmd{icc2\_shell} 中用 \cmd{update\_dc\_floorplan} 命令生成更新后的 floorplan。\\
        IC Compiler II 中的 floorplan 数据会传输到 DC Explorer，并且布局会被复位。
...
未传输 &
\begin{itemize}[leftmargin=1.2em,nosep]
  \item 层次模型：DC Explorer 与 IC Compiler 的块抽象与物理层次在 \cmd{icc2\_shell} 中视为未解析引用
...
默认情况下，路径 slack 直方图以 10 个 bin 显示所有路径的 slack 值分布，每个 bin 的最小范围为 100~ps。
...
可创建违及时序约束的同时钟路径、跨时钟路径、不可行路径或多周期路径的直方图。
默认情况下，逻辑级直方图以 8 至 16 个 bin 显示所有路径的逻辑级分布。
...
DC Explorer 在 GUI 中提供交叉探测能力，使您能在 RTL 浏览器中审阅、验证并检查 RTL 设计的对应关系。
...
若交叉探测之后 RTL 被移到不同位置，重复同一交叉探测序列将不再反映先前的行号；因此工具无法定位文件并发出错误消息。
