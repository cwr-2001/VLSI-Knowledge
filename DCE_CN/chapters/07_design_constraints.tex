...
logic library 定义这些 implicit constraint。
这些约束是设计正确工作必须满足的要求，适用于使用该 library 的任何 design。
默认情况下，design rule constraint 的优先级高于 optimization constraint。
...
单元退化是设计规则约束。
某些逻辑库包含单元退化表。
这些表给出 cell 可驱动的 maximum capacitance，其值是 cell input transition time 的函数。
...
\begin{itemize}
  ...
  \item 要约束 asynchronous path，使用 \cmd{set\_max\_delay} 与 \cmd{set\_min\_delay} 命令指定 point-to-point maximum/minimum delay。
\end{itemize}
...
DC Explorer 综合考虑 clock waveform 与 skew、library setup time、external delay、multicycle/false-path specification，以及 \cmd{set\_max\_delay} 设置的值，确定设计中每条 timing path 的 maximum-delay target。
...
DC Explorer 按下列优先级降序解决 design rule constraint 之间的冲突。
\begin{enumerate}
  \item 最小电容
  \item 最大转换时间
  \item 最大扇出
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item 对使用 nonlinear delay model 的 library，DC Explorer 通过 table lookup 和 interpolation 计算 transition time；该值是 output pin capacitance 和 input transition time 的函数。
  \end{itemize}
...
该负载的单位必须与逻辑库一致。
该值用于 timing optimization，不用于 max-fanout optimization。
...
compile 期间，DC Explorer 确保 net transition time 小于指定值；例如，可在 driving gate 的输出端插入 buffer。
...
该频率定义为驱动某一 logic cone 的 register 所使用的最高 clock frequency。
...
被驱动 cell（U3）施加的 fanout load 不一定为 1.0。
library 可以定义更高的 fanout load，以建模 cell 的内部 fanout effect。
也可在 output port（OUT1）上设置 fanout load，以建模 external fanout effect。
...
lower-level block 与 top-level block 发生冲突时，适用下列规则：
\begin{itemize}
  \item \textbf{时钟名冲突}\\
  ...
  \item \textbf{来自或指向未传播时钟的例外}\\
        该时钟可以是 virtual clock，也可以是因冲突而未从该 block 传播的 clock。
  \item \textbf{例外}\\
