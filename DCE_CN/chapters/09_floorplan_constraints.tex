...
下例写出所有已布局标准单元到 \texttt{placed\_std.fp} 风格文件：
\begin{lstlisting}
...
\cmd{get\_placement\_blockages} 返回当前设计中的 blockage 集合；可用 \opt{-within} 指定矩形或多边形区域，或用 \opt{-filter} 过滤：
\begin{lstlisting}
...
\cmd{get\_bounds} 返回 bound 集合；默认最多显示 100 个对象（可用 \varname{collection\_result\_display\_limit} 更改）：
\begin{lstlisting}
...
\cmd{create\_user\_shape} 命令保存在 floorplan 文件的 Preroute 段。用 \cmd{remove\_user\_shape *} 移除全部 user shape。
...
用 \cmd{create\_track} 为布线层与 polygon 层创建 track。Track 覆盖整个 die area；若 die area 为多边形，track 以矩形覆盖并可能伸出多边形外。
必须为 track 指定 polygon 层或布线层；在 polygon 层上创建 track 并不支持在该层布线。
...
\cmd{get\_tracks} 按条件返回 track 集合；\cmd{report\_track}（无选项）报告金属层、方向、起点、数量、pitch 与属性来源。
...
用 \cmd{set\_keepout\_margin} 为指定单元或库单元创建 keepout margin；也可指定 \opt{-tracks\_per\_macro\_pin} 自动推导：
\begin{lstlisting}
...
可在 floorplan 中包含无逻辑功能的物理单元，称为仅物理单元（physical-only cell），例如 filler、tap、flip-chip pad、endcap、decap。
...
默认工具在所有仅物理单元上设置 \texttt{is\_physical\_only}。检查某单元是否为仅物理单元：
\begin{lstlisting}
...
报告方式：
\begin{itemize}
  \item \cmd{report\_cell -only\_physical} — 报告当前设计的仅物理单元
