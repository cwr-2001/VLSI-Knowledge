...
DC Explorer 使用 technology library、symbol library 以及 synthetic library 或 DesignWare library 来实现综合，并以图形方式显示综合结果。
必须正确设置这些 library，DC Explorer 才能正确使用其中的数据。
...
Logic library 由半导体厂商维护和发布，包含每个 cell 的特性与功能信息，例如 cell name、pin name、面积、delay arc 和 pin loading。
Logic library 还定义必须满足的条件，例如 net 的 maximum transition time；这些条件称为 design rule constraints。
此外，logic library 还为特定工艺指定 operating conditions 和 wire load models。

DC Explorer 支持使用 nonlinear delay model（NLDM）、Composite Current Source（CCS）model（compact 或 noncompact），或同时包含 NLDM 和 CCS model 的 logic library。
工具根据 logic library 的内容自动选择 timing model 类型。若 library 同时包含 NLDM 和 CCS model，DC Explorer 使用 CCS model。
在 logic synthesis 和 preroute optimization 期间，工具可能不会使用所有可用的 CCS 数据，以缩短 runtime。
...
\begin{itemize}
  \item \textbf{实现设计功能}\\
        优化期间 DC Explorer 映射到的 logic library 称为 target library。
        Target library 包含用于生成 netlist 的 cell，以及设计 operating conditions 的定义。
        用于 compile 或 translate 设计的 target library 会成为该设计的 local link library。
        DC Explorer 将此信息保存在设计的 \texttt{local\_link\_library} 属性中。

  \item \textbf{解析单元引用}\\
        DC Explorer 用于解析 cell reference 的 logic library 称为 link library。
        Link library 包含 mapped netlist 中 library cell 和 subdesign 的描述，也可包含 design file。
        Link library 包括 \texttt{local\_link\_library} 属性中定义的 local link library，以及 \varname{link\_library} 变量中指定的 system link library。
...
DC Explorer 将 \varname{link\_library} 变量中找到的第一个 logic library 作为 main library。
若其他 library 的度量单位与 main library 不同，DC Explorer 会将所有单位转换为 main library 中指定的单位。
...
\begin{itemize}
  \item \textbf{目标库}\\
        ...
        应仅指定希望 DC Explorer 用于映射设计中 standard cell（如组合逻辑和寄存器）的 standard cell library。
        不应指定任何 DesignWare library 或 macro library（如 pad 或 memory）。
  ...
  \item \textbf{链接库}\\
        ...
        \begin{itemize}
          \item 系统变量 \varname{link\_library} 列出 DC Explorer 用于解析引用的库与设计文件。\\
                ...
                在 \varname{link\_library} 中指定星号，表示 DC Explorer 在内存中已加载的 library 中搜索 reference。
          ...
          \item \varname{search\_path} 变量指定目录路径列表：当您仅指定不含路径的普通文件名时，工具用其查找逻辑库及其他文件。
                该变量还指定 DC Explorer 搜索完 link library 后，可继续搜索 unresolved reference 的路径。
        \end{itemize}
\end{itemize}
...
Symbol library 由半导体厂商维护和发布，包含 design schematic 中表示 library cell 的图形符号定义。
生成 design schematic 时，DC Explorer 将 netlist 中的 cell 一对一映射到 symbol library 中的 cell。要查看 design schematic，请使用 Design Vision。

\subsection{DesignWare 库}
\subsubsection*{DesignWare Libraries}

DesignWare library 是可重用 intellectual property（IP）building block 或 component 的集合，并紧密集成到 Synopsys 综合环境中。
...
若要用 DC Explorer 生成可在 IC Compiler design planning 中启动物理探索的早期 netlist，除 logic library 外，还需指定 physical library。
使用 Milkyway design library 指定 physical library，并以 Milkyway 格式保存设计。
...
\begin{itemize}
  \item \textbf{Milkyway 参考库}\\
        ...
        Reference library 还定义 placement unit tile，即最小可布局 instance 的宽度和高度，以及 routing direction。
...
若执行 technology translation，请将现有 mapped gate 所使用的 standard cell library 加入 link library，并将转换目标 standard cell library 加入 target library。
...
例如，要使用 DesignWare minPower component，请在 \varname{synthetic\_library} 的设置中包含 \texttt{dw\_minpower.sldb} library。此操作不会执行功耗优化。
...
\end{lstlisting}
        若不设置这些变量，执行 \cmd{write\_milkyway} 时不会建立 power 和 ground connection；power net 和 ground net 可能会被转换为 signal net。
...
Physical variant cell 与 master cell 的差异在于 mask color 或细微的物理几何形状。
对于先进工艺节点，这些 cell 的 timing characteristic 不会发生变化，因此 master cell 及其 variant 可使用同一个 Liberty model。
...
若 library 不是相应的 binary format，请使用 \cmd{read\_lib} 命令编译 library source。\cmd{read\_lib} 命令需要 Library-Compiler license。
...
Library object 是 vendor-specific cell 及其 pin。指定 library object 时，使用下列命名约定：
\begin{lstlisting}
...
\end{lstlisting}
该命令仅影响当前加载到内存中的 library 副本，对磁盘上的版本没有影响。
但是，如果保存该 library，排除设置也会被保存，相关 cell 将被永久排除。
...
Logic library 与 physical library 之间的一致性对获得良好结果至关重要。处理设计之前，请运行 \cmd{check\_library} 命令以确保 library 一致。
\begin{lstlisting}
...
默认情况下，优化期间工具可从 target library 中选择任意 library cell。
要将 library cell 限制为 target library 的一个 subset，或针对特定 design block 过滤 target library cell，请使用 \cmd{set\_target\_library\_subset} 命令。
例如，可在优化期间对某些设计块从目标库中排除特定双高度单元。

使用 \cmd{set\_target\_library\_subset} 时请注意：
\begin{itemize}
  \item Subset restriction 仅适用于优化期间新建或映射的 cell。除非工具在优化期间修改已 mapped cell，否则该限制不影响这些 cell。
  \item 该命令将 subset restriction 应用于指定 design 及其 hierarchy 中的 subdesign，但已设置其他 subset restriction 的 subdesign 除外。
  \item 较低 hierarchy level 指定的 subset 会覆盖较高 level 指定的任何 subset。
  \item 由该命令设置的子设计不能被 \cmd{ungroup} 命令或工具的自动 ungroup 能力解除分组。
  \item Subset restriction 仅适用于 current design。
\end{itemize}
...
当用 \cmd{set\_tlu\_plus\_files} 为 5~nm 及更小的 technology node 读取 nxtgrd 文件时，必须持有 DC-Synth-AdvGeo license。
若许可证不可用，工具发出错误消息。
...
\begin{itemize}
  \item \textbf{Functionally unknown black box}\\
        逻辑功能未知的 cell。示例包括：
        \begin{itemize}
          \item 宏单元
          \item 空层次单元或已黑盒化的模块
  ...
  \item \textbf{Logical black box}\\
        未链接到 logic library 中某个 cell 的 cell。此类 black box 归入 functionally unknown black box。示例包括：
        \begin{itemize}
          \item 空层次单元或已黑盒化的模块
          \item 未链接或未解析的单元
        \end{itemize}
  ...
  \item \textbf{Physical black box}\\
        没有 physical representation 的 cell。示例包括：
        \begin{itemize}
          \item 空层次单元或已黑盒化的模块
          \item 未链接或未解析的单元
        \end{itemize}
...
DC Explorer 支持根据 physical black box 被真实逻辑替换后所含 object 的估算结果，设置该 black box cell 的尺寸。
还可定义 gate equivalence calculation 的 base unit area，以估算 black box 的尺寸。
...
Quick timing model 是一种近似 timing model，适合在设计周期早期描述 black box 的粗略初始时序。
