...
DC Explorer 从设计文件将设计读入内存。任意时刻内存中可有多个设计。
...
引用使您能在单个设计中优化每个单元（如 NAND 门），而不影响其他设计中的单元。
...
实例（instance）是已加载到内存中的引用（库组件或设计）在电路中的一次出现；每个实例有唯一名称。
一个设计可包含多个实例。
...
若随后打开同名文件的设计，DC Explorer 会覆盖原设计。
...
参数 & 允许在 \cmd{elaborate} 命令行设置参数值。对参数化设计，可用 \cmd{analyze}/\cmd{elaborate} 以非默认值构建新设计。 & 不能传递参数；必须在 HDL 中使用 directive。 \\
...
仅当指定的引用组件或设计与原引用具有相同数量、相同大小与方向的端口时，DC Explorer 才会更改链接。
...
例如，要在 Count\_16 设计中的 U1/U15 层次单元上放置 \texttt{dont\_touch} 属性，输入：
\begin{lstlisting}
...
属性描述设计数据库中对象的逻辑、电气、物理及其他特性。
属性附着于设计对象并随设计数据库保存，但修改后的属性设置不会自动保存。
...
\end{lstlisting}
  \item 使用 \cmd{reset\_design} 命令从当前设计移除所有属性\\
        \cmd{reset\_design} 移除所有设计信息，包括时钟、输入与输出延时、路径组、工作条件、时序范围与线负载模型。
        运行该命令通常等效于重新开始设计。
  \item 更改属性值\\
...
要指定综合流程中应在时序单元上保留哪些用户定义属性，使用 \cmd{define\_preserve\_user\_attribute} 命令。
...
默认情况下，\cmd{remove\_design} 仅从 \cmd{de\_shell} 内存移除指定设计。
...
\begin{noteBox}
每当要写出 Verilog 或 VHDL 设计时，始终使用
\cmd{change\_names} \opt{-rules} \opt{-[verilog|vhdl]} \opt{-hierarchy} 命令，
因为设计数据库文件中的命名不符合 Verilog 或 VHDL。例如：
\begin{lstlisting}
