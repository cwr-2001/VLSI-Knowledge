...
相对布局（relative placement，RP）是 IC Compiler II 中用于精确控制一组单元之间相对列、行位置关系的布局约束机制。设计者可以把若干单元组织成一个矩阵状的“相对布局组”，在后续粗布局（coarse placement）、优化、时钟树综合与绕线过程中，工具会把该组中的单元当作一个整体来维护其内部拓扑，不会随意打散重排。业界也常把这种能力称为“物理数据通路”（physical datapath）或“结构化布局”（structured placement）。
...
\begin{lstlisting}
icc2_shell> add_to_rp_group rp1 -blockage gap1 \
...
\begin{lstlisting}
icc2_shell> add_to_rp_group rp2 -cells U41 -free_placement \
...
\begin{lstlisting}
icc2_shell> create_rp_group -name rp1 -columns 1 -rows 4
icc2_shell> set_rp_group_options rp1 -alignment right
icc2_shell> add_to_rp_group rp1 -cells U0 -column 0 -row 0 \
...
\begin{lstlisting}
icc2_shell> add_to_rp_group rp -cells U5 \
...
\begin{lstlisting}
icc2_shell> set_rp_group_options [get_rp_groups design::rp] \
...
\begin{lstlisting}
icc2_shell> set_rp_group_options rp2 \
...
IC Compiler II 图形界面提供了专门用于可视化、分析相对布局组的工具，包括 Relative Placement (RP) Groups Visual Mode 与 Relative Placement (RP) Net Connection Visual Mode 两种可视化模式。更详细的用法说明请参阅《IC Compiler II Graphical User Interface User Guide》中关于 Map 与 Visual Modes 的相关主题。
