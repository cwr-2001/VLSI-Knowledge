...
IC Compiler II 工具使用设计库（design library）来存储设计及其相关库信息。本节说明如何创建设计库，以及如何准备并保存设计。
...
\begin{itemize}
  ...
  \item 处理设计（Working With Designs）
  \item 导入 floorplan 信息
  \item 设置多电压设计（Setting Up Multivoltage Designs）
  \item 指定时序约束与设置
  \item 指定逻辑设计规则约束
  \item 控制时钟门控延迟（Clock-Gate Latencies）
  \item 为布局与合法化指定物理约束
  \item 指定布局设置
  \item 指定合法化设置
...
IC Compiler II 工具使用搜索路径（search path）查找以相对路径指定、或未指定路径的文件。
...
\begin{seeAlsoBox}
更多信息见 \textit{IC Compiler II Data Model User Guide} 中的 Design Libraries 主题。
\end{seeAlsoBox}
...
在库配置过程中：
\begin{itemize}
  \item IC Compiler II 工具自动调用 Library Manager，无需用户干预即可生成单元库，如下图所示：
\end{itemize}

\begin{figure}[htbp]
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书示意：.db / .frame / ASCII technology file $\rightarrow$ Cell libraries $\rightarrow$ Synthesis）}\\[0.3em]
\small 库配置流程示意；请参阅原版 PDF。
\vspace{1.2cm}}}
\caption{库配置中生成单元库}
...
\begin{seeAlsoBox}
更多信息见 \textit{IC Compiler II Data Model User Guide} 中的 Configuring Cell Libraries 主题。
\end{seeAlsoBox}

\subsection{限制库单元使用}
\subsubsection*{Restricting Library Cell Usage}

默认情况下，对 block 执行优化或时钟树综合时，IC Compiler II 可使用单元库中全部可用库单元。要限制单元使用，在 block 已载入内存后使用 \cmd{set\_lib\_cell\_purpose}。该命令具有 block 作用域，并在工具会话间保持；重新打开 block 时会恢复这些设置。
...
\begin{lstlisting}
icc2_shell> set_lib_cell_purpose -include none lib_cells
icc2_shell> set_lib_cell_purpose -include cts lib_cells
\end{lstlisting}

允许某组库单元用于除时钟树综合外的所有用途：
\begin{lstlisting}
icc2_shell> set_lib_cell_purpose -exclude cts lib_cells
\end{lstlisting}
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 6：Logic Hierarchy of Design）}
\vspace{1.2cm}}}
\caption{设计的逻辑层次}
...
为此使用下列设置：
\begin{lstlisting}
icc2_shell> set_target_library_subset -objects {top/Sub1} \
...
若此前使用基于 Tcl 的旧版本，可用 \cmd{translate\_pin\_check\_ui} 将旧脚本转换为新的 IC Compiler II 命令：
\begin{lstlisting}
...
在多数情况下，在 IC Compiler II 中处理设计时，使用设计库中某一版本（block）的 design view。本手册中，\textbf{block} 指设计库中的特定设计版本（不论顶层设计还是子设计）；\textbf{design} 泛指正在处理的设计。除非另有说明，二者均指 design view。

IC Compiler II 以 Verilog 格式读取设计。Verilog 网表文件是单个文件中的结构级或门级设计。要将设计读入 IC Compiler II：
\begin{enumerate}
...
若 block 尚无 floorplan，可按 \textit{IC Compiler II Design Planning User Guide} 所述执行设计规划并生成 floorplan。
...
\begin{seeAlsoBox}
更多信息见 \textit{IC Compiler II Data Model User Guide} 中的 ``Rule-Based Name Matching''。
\end{seeAlsoBox}
...
\textit{（原书 Example 1：DEF Port Location Information）}
\vspace{0.8cm}}}
\caption{DEF Port 位置信息（示例占位）}
\label{ex:def-port-loc}
\end{figure}
...
默认情况下，IC Compiler II 中 \appopt{snps\_handle\_physical\_only} 变量为 \texttt{true}。因此加载并提交 UPF 时，这些约束会应用到仅物理单元。
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 7：Merging Voltage Area Shapes）}
\vspace{1.2cm}}}
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 8：Nested Voltage Areas）}
\vspace{1.2cm}}}
\caption{嵌套电压区域}
\label{fig:nested-va}
\end{figure}
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 9：Voltage Area Guard Band）}
\vspace{1.2cm}}}
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 10：Effective Boundaries of Overlapping Voltage Areas）}
\vspace{1.2cm}}}
\caption{重叠电压区域的有效边界}
\label{fig:eff-bound}
\end{figure}
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 11：Physical-Feedthrough Nets of Voltage Areas）}\\[0.3em]
\small Physical view / Logical view；VA0(PD0)、VA1(PD1)、N1、Block1(PD1)
\vspace{1.2cm}}}
...
\begin{lstlisting}
icc2_shell> create_voltage_area_rule -allow_pass_through false \
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 12：Physical-Feedthrough Nets Disabled for a Voltage Area）}
\vspace{1.2cm}}}
...
\begin{lstlisting}
icc2_shell> create_voltage_area_rule \
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 13：Difference in the Logical View After Physical- and Logical-Feedthrough Buffering）}
\vspace{1.2cm}}}
...
\begin{lstlisting}
icc2_shell> create_voltage_area_rule -default_rule \
   -allow_physical_feedthrough true
\end{lstlisting}
...
多电压设计在电源域接口处需要特殊的多电压单元，例如电平移位单元与隔离单元。电平移位单元用于工作在不同电压电平的电源域之间；隔离单元用于处于不同状态（关断相对 always-on 或上电）的电源域之间。通常多电压单元在逻辑综合期间插入；也可在 IC Compiler II 中插入。
...
隔离单元用于有选择地关断电源域电压接口的输入侧；它们不进行电平移位。隔离单元应在 RTL 级例化，以避免形式验证错误。也可使用 IC Compiler II 插入。
...
运行 \cmd{associate\_mv\_cells} 或 \cmd{commit\_upf} 时，IC Compiler II 会自动将功耗策略与已有多电压单元关联。若自动关联不正确，可用 \cmd{set\_power\_strategy\_attribute} 手动修改关联。要用 \cmd{get\_power\_strategies} 确定某电源域的功耗策略。
...
IC Compiler II 使用片上偏差（on-chip variation，OCV）模式进行时序分析，以建模芯片上工作条件变化的影响。该模式执行保守的时序分析，允许最小与最大延迟同时作用于不同路径。对 setup 检查，发射时钟路径与数据路径使用最大延迟，捕获时钟路径使用最小延迟；对 hold 检查，发射时钟路径与数据路径使用最小延迟，捕获时钟路径使用最大延迟。
...
详细信息见：
\begin{itemize}
  \item 时钟与时钟效应：\textit{IC Compiler II Timing Analysis User Guide} 中的 ``Defining Clocks''；
...
优化期间，IC Compiler II 会设法满足设计规则约束，即使这意味着违反时序、功耗与面积等优化约束——这些设计规则约束具有更高优先级。优化后，可用表~\ref{tab:drc-cmd} 中的报告命令识别 block 中的设计规则约束违例。
...
\begin{noteBox}
IC Compiler II 不遵从最大 fanout 设计规则约束。但可用 \appopt{opt.common.max\_fanout} 应用选项为 block 中的数据路径指定最大 fanout。这是软优化约束。优化期间工具会设法确保数据路径单元驱动的扇出不超过指定的最大 fanout。
\end{noteBox}
...
综合期间，IC Compiler II 假定时钟为理想时钟。理想时钟在时钟网络上不产生延迟。作出该假定是因为真实时钟网络延迟要到时钟树综合之后才可知。实际上时钟并非理想，时钟网络上存在非零延迟。对带时钟门控的设计，寄存器处的时钟网络延迟与时钟门控单元处的时钟网络延迟不同。寄存器与时钟门控单元之间的时钟网络延迟差异，会使时钟门控单元使能输入处的 setup 条件约束更紧。
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 14：Clock Latency With Clock-Gating Design）}\\[0.3em]
\small lat\_reg / lat\_cgtoreg；Clock-Gating Cell 与 Register
\vspace{1.2cm}}}
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 15：Clock-Gating Stages and Latency Calculations）}
\vspace{1.2cm}}}
\caption{时钟门控级数与延迟计算}
\label{fig:cg-stages}
\end{figure}
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 16：Latency Calculations With Varying Fanout）}
\vspace{1.2cm}}}
\caption{变化扇出下的延迟计算}
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 17：Placement Keepout Margins）}\\[0.3em]
\small Outer keepout margin / Inner keepout margin；Fixed cell；left/right/top/bottom
\vspace{1.2cm}}}
...
基于区域的布局障碍是矩形区域，其中不能放置单元，或对单元类型或数量加以限制。IC Compiler II 支持下列类型的基于区域的布局障碍：
\begin{itemize}
...
表~\ref{tab:bound-types} 列出 IC Compiler II 支持的布局 bound 类型。
...
\begin{lstlisting}
icc2_shell> set add1_cells [get_flat_cells add1/*]
icc2_shell> set mult1_cells [get_flat_cells mult1/*]
icc2_shell> create_placement_attraction -name add1_mult1 \
...
\begin{lstlisting}
icc2_shell> set U1_cells [get_flat_cells U1/*]
icc2_shell> create_placement_attraction -name U1 \
   -region {{0 0} {2000 1500}} $U1_cells
\end{lstlisting}
  \item 沿某直线放置单元（例如宏单元边缘）：给出直线两端坐标，例如：
\begin{lstlisting}
icc2_shell> set U2_cells [get_flat_cells U2/*]
icc2_shell> create_placement_attraction -name U2 \
   -region {{1000 200} {1000 800}} $U2_cells
\end{lstlisting}
  \item 围绕某位置放置单元（例如 port 位置）：给出该位置坐标，例如：
\begin{lstlisting}
icc2_shell> set U3_cells [get_flat_cells U3/*]
icc2_shell> create_placement_attraction -name U3 \
...
\begin{lstlisting}
icc2_shell> set_placement_spacing_rule \
...
\begin{lstlisting}
icc2_shell> set_app_options -name place.coarse.icg_auto_bound \
 -value true
\end{lstlisting}
...
\toprule
\cmd{set\_technology -node} & \appopt{pin\_cost\_aware} & \appopt{pin\_density\_aware} & \textbf{工具行为} \\
\midrule
7, 7+, 5, s5, or s4 & \texttt{true} & \texttt{true} 或 \texttt{false} & 执行工艺相关的 pin-cost 感知布局 \\
7, 7+, 5, s5, or s4 & \texttt{false} & \texttt{true} & 执行 pin 密度感知布局 \\
7, 7+, 5, s5, or s4 & \texttt{false} & \texttt{false} & 布局时不考虑单元 pin 影响 \\
其他任意设置 & \texttt{true} 或 \texttt{false} & \texttt{true} & 执行 pin 密度感知布局 \\
其他任意设置 & \texttt{true} 或 \texttt{false} & \texttt{false} & 布局时不考虑单元 pin 影响 \\
\bottomrule
\end{tabularx}
\end{table}
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 18：Default Cell Categories for IR-Drop-Aware Placement）}
\vspace{1.2cm}}}
...
\begin{lstlisting}
icc2_shell> set_placement_ir_drop_target VA1 high 1.5
icc2_shell> set_placement_ir_drop_target VA1 low 4.5
\end{lstlisting}

将 VA2 中压降大于电源电压 10\% 的单元归入上层，5\%--10\% 归入中层：
\begin{lstlisting}
icc2_shell> set_placement_ir_drop_target VA2 high 10 -irdrop
icc2_shell> set_placement_ir_drop_target VA2 low 5 -irdrop
\end{lstlisting}
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 19：Equivalent Cells With Different Pin Locations）}
\vspace{1.2cm}}}
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 20：Equivalent Cells With Pins of Different Colors）}
\vspace{1.2cm}}}
...
在 Library Manager 或 IC Compiler II 中，可用 \cmd{report\_cell\_groups} 报告库中的等效单元组；该命令既报告 variant-ready 逻辑库 Liberty 中定义的组，也报告 Library Manager 中用 \cmd{create\_cell\_groups} 定义的组。
...
\begin{lstlisting}
icc2_shell> analyze_lib_cell_placement -lib_cells [add_to_collection \
...
\begin{lstlisting}
icc2_shell> set_dont_touch [get_cells A] true
\end{lstlisting}

移除该属性可用 \cmd{remove\_attributes}，或将 \cmd{set\_dont\_touch} 设为 \texttt{false}：
\begin{lstlisting}
icc2_shell> remove_attributes [get_cells A] dont_touch
icc2_shell> set_dont_touch [get_cells A] false
\end{lstlisting}
...
查询：
\begin{lstlisting}
icc2_shell> get_attribute [get_cells cell_name] size_only
\end{lstlisting}
...
仅对 CLK 时钟路径：
\begin{lstlisting}
icc2_shell> set_dont_touch_network [get_clocks CLK] -clock_only
\end{lstlisting}

移除：
\begin{lstlisting}
icc2_shell> set_dont_touch_network [get_clocks CLK] -clear
\end{lstlisting}
...
例如，对全部此类 net 启用 DRC：
\begin{lstlisting}
icc2_shell> set_auto_disable_drc_nets -none
\end{lstlisting}

对连接时钟与常数的 net 禁用 DRC：
\begin{lstlisting}
icc2_shell> set_auto_disable_drc_nets \
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 21：DRC Disabled Clock and Constant Nets Highlighted）}\\[0.3em]
\small 图中高亮显示已禁用 DRC 的时钟 net 与常数 net。
\vspace{1.2cm}}}
...
默认情况下，若结果电路功能等价，优化可在 sizing 时更改叶子单元的 pin 名称。IC Compiler II 会自动更新内部约束版本以反映异常约束中涉及的新 pin 名。若发生此情况，用于约束 block 的原始约束不能再用于 signoff；必须从 IC Compiler II 写出约束并随结果 block 一并使用。
...
\begin{lstlisting}
icc2_shell> set_freeze_ports -clock [get_cells MBX22] true
\end{lstlisting}
...
\end{itemize}
\begin{lstlisting}
icc2_shell> set_isolate_ports {in3 out1}
icc2_shell> set_isolate_ports -driver LIBCELL_BUFF out3 -force
icc2_shell> set_isolate_ports -type inverter out4
\end{lstlisting}

\begin{figure}[htbp]
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书插图：Without port isolation / After running the port isolation commands）}\\[0.3em]
\small 示出插入 buffer 与 inverter 对前后的设计对比。
\vspace{1.2cm}}}
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{0.8cm}
\textit{（原书示意：Feedthrough net / Logically equivalent outputs）}
\vspace{0.8cm}}}
...
阻止向顶层 module 添加：
\begin{lstlisting}
icc2_shell> set_allow_new_cells \
   [get_attribute [current_block] top_module] false
\end{lstlisting}

阻止向 \texttt{M1} module 添加：
\begin{lstlisting}
icc2_shell> set_allow_new_cells [get_modules M1] false
\end{lstlisting}

阻止向 \texttt{U22} 层次单元添加：
\begin{lstlisting}
icc2_shell> set_allow_new_cells [get_cells U22] false
\end{lstlisting}

阻止向 \texttt{VA1} voltage area 添加：
\begin{lstlisting}
icc2_shell> create_voltage_area_rule -name VA1_rule \
...
\begin{lstlisting}
icc2_shell> set_via_ladder_candidate [get_lib_pins lib1/AND2/A1] \
   -ladder_name "VP4"
icc2_shell> set_via_ladder_candidate [get_lib_pins lib1/AND2/A1] \
   -ladder_name "VP2"
\end{lstlisting}
...
\begin{lstlisting}
icc2_shell> set_attribute [get_lib_pins lib1/INV4/A] \
...
\begin{lstlisting}
icc2_shell> set_attribute [get_via_rules EMVP1] \
...
要在 IC Compiler II 中使用 RTL SAIF 文件，须先在 Design Compiler 中完成：
\begin{enumerate}
...
然后在 IC Compiler II 中：
\begin{enumerate}
  \item 用 \cmd{saif\_map -read\_map} 读入 Design Compiler 的 name-mapping 文件，创建 name-mapping 数据库并开始跟踪对象名变更。\\
        若不读 name-mapping 文件、但希望跟踪 IC Compiler II 内的网表变更，可用 \cmd{saif\_map -start} 创建数据库并开始跟踪。
...
若 SAIF 文件中的时钟频率与 IC Compiler II 中不同，可按下列步骤缩放 switching activity：
\begin{enumerate}
  ...
  \item 用 \cmd{set\_power\_clock\_scaling} 缩放 switching activity。须指定：
        \begin{itemize}
          ...
          \item 用 \opt{-period} 指定 SAIF 中使用的时钟周期，或用 \opt{-ratio} 指定 SAIF 周期与 IC Compiler II 周期之比
        \end{itemize}
...
IC Compiler II 可执行基于 LVT 百分比的优化。限制 block 中 LVT 单元百分比可改善良率；也可在早期设计阶段用于评估 RTL 质量。

设置基于 LVT 百分比的优化：
\begin{enumerate}
  \item 指定 LVT 库单元组，例如：
\begin{lstlisting}
icc2_shell> remove_attributes -quiet \
 [get_lib_cells -quiet */*] threshold_voltage_group
icc2_shell> set_attribute [get_lib_cells -quiet *lvt*/*] \
   threshold_voltage_group LVT
icc2_shell> set_threshold_voltage_group_type \
   -type low_vt LVT
\end{lstlisting}
        确保所指定的 LVT 库单元：
        \begin{itemize}
...
IC Compiler II 中的 PrimePower In-Design 流程使用 RTL FSDB 的输入波形信息，执行基于时间的分析以更新 activity，提高功耗估计精度。
...
\begin{itemize}
  \item 调用 PrimePower，读入网表、库、PVT、约束、寄生、必要 name mapping、UPF 等设计数据。库细节来自 IC Compiler II 内存中的设计数据库。
  \item 将 RTL FSDB 读入 PrimePower，执行基于时间的分析以生成门级 SAIF。
  \item 将门级 SAIF 读回 IC Compiler II。
\end{itemize}
...
PrimePower In-Design UI 允许用原生 ZeBu 数据库（ZTDB）传递输入波形。\cmd{set\_indesign\_primepower\_options} 的 \opt{-ztdb} 选项配置 PrimePower In-Design，以用 \cmd{update\_indesign\_activity} 创建脚本；该脚本使用 PrimePower 的 \cmd{read\_ztdb} 做基于时间的分析，并生成自动回标到 IC Compiler II 的门级 SAIF。
...
IC Compiler II 提供下列技术保障设计电源完整性：
\begin{itemize}
...
设置推荐电源完整性流程：
\begin{enumerate}
  \item 将 \appopt{opt.common.power\_integrity} 设为 \texttt{true} 以启用。启用后，IC Compiler II 在不同阶段使用：
        \begin{enumerate}
...
\begin{lstlisting}
icc2_shell> set_app_option -name opt.common.power_integrity \
  -value false
icc2_shell> set_app_option -name place_opt.flow.enable_dps \
  -value true
\end{lstlisting}
...
icc2_shell> set_app_option -name opt.common.power_integrity \
  -value false
icc2_shell> set_app_option -name place_opt.flow.enable_irap \
  -value true
\end{lstlisting}
...
icc2_shell> set_app_option -name opt.common.power_integrity \
  -value false
icc2_shell> set_app_option -name clock_opt.flow.enable_irap \
  -value true
\end{lstlisting}
\end{itemize}
...
\begin{lstlisting}
icc2_shell> set_routing_rule [get_nets n1] \
   -min_routing_layer M2 -max_routing_layer M7
\end{lstlisting}
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 22：Root, Internal, and Sink Clock Net Types）}
\vspace{1.2cm}}}
...
\begin{lstlisting}
icc2_shell> set_clock_routing_rules -clocks CLK1 -net_type root \
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 23：Using a Fanout Limit for Selecting Root Nets）}
\vspace{1.2cm}}}
...
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{1.2cm}
\textit{（原书 Figure 24：Level-Based Clock Nondefault Routing Rule）}
\vspace{1.2cm}}}
...
启用该功能：
\begin{lstlisting}
icc2_shell> set_app_options -list {cts.compile.topological_ndr true}
\end{lstlisting}

\subsection{为层设置优选布线方向}
\subsubsection*{Setting the Preferred Routing Direction for Layers}

IC Compiler II 要求为工艺文件中定义的布线层指定优选布线方向。通常该信息在单元库中定义。要设置或更改某层的优选布线方向，按下列语法设置其 \texttt{routing\_direction} 属性：
\begin{lstlisting}
...
设计周期早期迭代中，设计数据可能不完整或不正确，从而妨碍探索性实现等。IC Compiler II Early Data Check Manager 允许指定工具在下列实现任务中如何处理数据检查：
\begin{itemize}
  \item 设计规划
  \item 多电压设计实现
  \item 优化
...
\begin{lstlisting}
icc2_shell> set_early_data_check_policy -policy strict \
...
icc2_shell> report_early_data_checks -check place.coarse.missing_scan_def
...
\begin{lstlisting}
icc2_shell> set_early_data_check_policy -policy lenient \
    -check place.coarse.*
icc2_shell> create_placement
Information: Policy for early data check 'place.coarse.missing_scan_def'
 is 'tolerate'. (EDC-001)
...
更多信息见 \textit{IC Compiler II Data Model User Guide}。
