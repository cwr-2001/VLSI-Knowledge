...
\begin{questionBox}
\textbf{问题 1.}\ 当前分析的设计名称是什么？\\
\textbf{问题 2.}\ ORCA 有多少个 setup 违例与 hold 违例？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 3.}\ 其中有多少个 reg-reg 类型的 setup 与 hold 违例？
\end{questionBox}
\end{enumerate}
...
\begin{questionBox}
\textbf{问题 5.}\ 根据上一条命令，\cmd{restore_session} 是否接受开关（switches）？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 6.}\ ORCA 设计的时序报告（以及所有其他报告）使用的时间单位是什么？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 7.}\ \cmd{link_path} 中的 4 个库是否都已成功读入 PrimeTime？\\
\textbf{问题 8.}\ 哪一个库定义了时间单位、工作条件（operating conditions）以及其他延迟计算相关信息的默认值？\\
\textbf{问题 9.}\ 使用的时间单位是什么？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 10.}\ 是否存在未反标的网？\\
\textbf{问题 11.}\ 作为调试缺失网的“下一步”，\cmd{report_annotated_parasitics} 的哪个选项比较合适？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 12.}\ 作为调试缺失约束的“下一步”，\cmd{check_timing} 的哪个选项比较合适？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 13.}\ 许多时序检查处于 untested 状态——这是否合理？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 15.}\ 哪一步占用的 CPU 时间最多？\\
\textbf{问题 16.}\ 哪些命令引发了 UITE-214 消息？\\
\textbf{问题 17.}\ 是否有部分 timing update 可以避免？\\
\textbf{问题 18.}\ 为什么在通读日志之前，从 \cmd{quit} 命令的输出入手可能是个好起点？
\end{questionBox}
\end{enumerate}
...
\begin{questionBox}
\textbf{问题 20.}\ 至少有 4 处线索表明这是 setup 而非 hold 报告。你能找出多少？\\
\textbf{问题 21.}\ 写出起点与终点触发器的实例名。\\
\textbf{问题 22.}\ 该时序路径的时钟偏斜（clock skew）为 0.511\,ns；可用报告中的哪两行计算得到？
\end{questionBox}

\begin{questionBox}
\textbf{问题 23.}\ 该时钟偏斜如何影响松弛（slack）——有助于还是有害于 slack？\\
\textbf{问题 24.}\ 违例相对于时钟周期有多大？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 25.}\ 至少有 4 处线索表明这是 hold 而非 setup 报告。你能找出多少？\\
\textbf{问题 26.}\ 在本 hold 报告中，时钟偏斜如何影响 slack——有助于还是有害于 slack？
\end{questionBox}
...
\section{答案 / 解答}
\label{lab1:solutions}

\noindent\textit{Answers / Solutions}

\begin{answerBox}
\textbf{问题 1.}\ 设计名称为 \textbf{ORCA}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 2.}\ 共有 \textbf{23} 个 setup 违例与 \textbf{53} 个 hold 违例。
\end{answerBox}

\begin{answerBox}
\textbf{问题 3.}\ 类型为 reg-reg 的违例为：\textbf{9} 个 setup、\textbf{53} 个 hold。
\end{answerBox}

\begin{answerBox}
\textbf{问题 4.}\ 命令 \cmd{!2} 重复历史中第 2 条已执行命令；命令 \cmd{!!} 重复历史中最近一条已执行命令。
\end{answerBox}

\begin{answerBox}
\textbf{问题 5.}\ 是。它接受会话名等参数/开关，从而可指定恢复众多已保存会话中的哪一个。
\end{answerBox}

\begin{answerBox}
\textbf{问题 6.}\ \textbf{1\,ns}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 7.}\ 是——用 \cmd{link_library} 指定的四个库与 \cmd{list_libraries} 显示的一致。
\end{answerBox}

\begin{answerBox}
\textbf{问题 8.}\ \cmd{cb13fs120_tsmc_max}——\cmd{list_libraries} 输出中库名前的星号表示“主库”。
\end{answerBox}
...
\begin{answerBox}
\textbf{问题 10.}\ 是：存在一些内部无驱动网（driverless nets），以及一些边界 pin-to-pin 网未反标。
\end{answerBox}

\begin{answerBox}
\textbf{问题 11.}\ \cmd{report_annotated_parasitics -help} 可列出选项；
\opt{-list_not_annotated} 可列出网名；
若关注 pin-to-pin 网，可使用 \opt{-pin_to_pin_nets}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 12.}\ \cmd{check_timing -help} 列出选项；\opt{-verbose} 可列出未约束端点（unconstrained endpoints）。
\end{answerBox}

\begin{answerBox}
\textbf{问题 13.}\ 合理——\cmd{test_mode}、\cmd{scan_en} 与 \cmd{power_save} 被约束为关；
对额外模式做 STA 可能使更多检查变为 tested。
\end{answerBox}

\begin{answerBox}
\textbf{问题 14.}\ 没有。该运行脚本的设计使得一旦出错会在中途终止；若脚本完整跑完，则执行过程中未发生错误。
此外，日志末尾由 \cmd{quit} 给出的 Diagnostics Summary 也显示本次运行无错误。
\end{answerBox}

\begin{answerBox}
\textbf{问题 15.}\ 对本实验而言，\textbf{source 约束}占用的 CPU 时间最多。
\end{answerBox}

\begin{answerBox}
\textbf{问题 16.}\ \cmd{update_timing -full}，以及调用宏 \cmd{PLL_SHIFT}（其内部触发了 \cmd{update_timing}）。
\end{answerBox}

\begin{answerBox}
\textbf{问题 17.}\ 可以。在 \cmd{set_propagated_clocks} 前后各有一次 \cmd{update_timing}——
...
\begin{answerBox}
\textbf{问题 18.}\ 它给出潜在问题点的高层摘要：警告/信息消息、timing update，以及性能统计。
\end{answerBox}

\begin{answerBox}
\textbf{问题 19.}\ ORCA 中有 \textbf{6} 个时钟。
\end{answerBox}

\begin{answerBox}
\textbf{问题 20.}\ 最醒目的线索是报告中的 \textbf{library setup time}。更细微的线索包括：
\begin{itemize}
...
\begin{answerBox}
\textbf{问题 22.}\ 可用启动端与捕获端的 \texttt{clock network delay (propagated)} 两行：
$3.247 - 2.736 = 0.511$\,ns。
\end{answerBox}

\begin{answerBox}
\textbf{问题 23.}\ 在本条 setup 报告中，时钟偏斜\textbf{有害于} slack：
启动端触发器的时钟延迟使数据晚到约 0.511\,ns，从而加大 setup 违例。
\end{answerBox}

\begin{answerBox}
\textbf{问题 24.}\ 时钟周期为 8\,ns，违例约 0.987\,ns，大约相当于周期的 \textbf{12\%}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 25.}\ 最醒目的线索是报告中的 \textbf{library hold time}。更细微的线索包括：
\begin{itemize}
...
\begin{answerBox}
\textbf{问题 26.}\ 从报告可见（施加 clock network delay 之后），捕获时钟边晚于启动时钟边到达，
