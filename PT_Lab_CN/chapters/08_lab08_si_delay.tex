...
\begin{enumerate}
  \item 回答下列问题：
\begin{questionBox}
\textbf{问题 1.}\ 将“常规”PrimeTime 运行脚本修改为 PrimeTime SI 运行脚本，必须包含哪两个步骤？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 2.}\ 本设计有多少个违例？\\
\textbf{问题 3.}\ 本设计中 setup 的最坏 slack 有多大？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 4.}\ 该路径上 delta delay 最大的网是哪条？最坏 delta delay 有多大？\\
\textbf{问题 5.}\ 该 delta delay 如何表示（时间单位还是对 VDD 的比值）？\\
\textbf{问题 6.}\ delta delay 为正还是为负？是否符合你的预期？\\
\textbf{问题 7.}\ 与该大 delta delay 对应的 stage delay 是多少？\\
\textbf{问题 8.}\ 报告中 ``\texttt{\&}'' 符号的含义是什么？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 9.}\ delta delay 为正还是为负？是否符合你的预期？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 10.}\ 本设计中最大的 delta delay 是多少？\\
\textbf{问题 11.}\ 报告的网是 victim 还是 aggressor？\\
\textbf{问题 12.}\ 报告的网是否位于违例时序路径上？
\end{questionBox}
...
（提示：参阅 Job Aid 或 \cmd{report_si_bottleneck} 的 man 页。）
\end{questionBox}

  \item 参照 Job Aid，查找并执行合适命令，找出 delta delay 与 stage delay 比值最大的 victim 网：
\begin{questionBox}
\textbf{问题 15.}\ 该报告中的 cost 表示什么（时间单位还是比值）？\\
\textbf{问题 16.}\ 报告的网是 victim 还是 aggressor？\\
\textbf{问题 17.}\ 如何减小这些网上的 delta delay？（提示同上。）
\end{questionBox}

  \item 参照 Job Aid，查找并执行合适命令，找出本设计中最强的 aggressor 网：
\begin{questionBox}
\textbf{问题 18.}\ 该报告中的 cost 表示什么（时间单位还是对 VDD 的比值）？\\
\textbf{问题 19.}\ 报告的网是 victim 还是 aggressor？\\
\textbf{问题 20.}\ aggressor 网本身是否一定位于违例时序路径上？\\
\textbf{问题 21.}\ 如何减小这些 aggressor 网引起的 delta delay？（提示同上。）
\end{questionBox}

  \item 参照 Job Aid，查找并执行合适命令，报告最坏时钟偏斜及相关的串扰贡献：
\begin{questionBox}
\textbf{问题 22.}\ 该报告是否表明时钟网屏蔽良好？
\end{questionBox}
\end{enumerate}
...
\section{答案 / 解答}
\label{lab8:solutions}

\noindent\textit{Answers / Solutions}

\begin{answerBox}
\textbf{问题 1.}\ 必须包含的两个步骤：
\begin{lstlisting}
# 在运行脚本开头设置该变量
...
\begin{answerBox}
\textbf{问题 4.}\ 该路径上最大 delta delay 为 \textbf{0.1024\,ns}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 5.}\ 以\textbf{时间单位}表示；主库时间单位为 ns。
\end{answerBox}

\begin{answerBox}
\textbf{问题 6.}\ delta delay 为\textbf{正}。合理——这是 setup 报告，数据路径上应考虑最坏情况下的串扰\textbf{减速}效应。
\end{answerBox}

\begin{answerBox}
\textbf{问题 7.}\ stage delay 为单元 + 网延迟：$0.5522 + 0.1025 = 0.6547$\,ns；\\
delta delay 与 stage delay 之比约为 \textbf{15\%}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 8.}\ 表示延迟使用详细网寄生数据计算。符号含义（摘自 \cmd{report_timing} man 页）：
\begin{center}
\small
\begin{tabular}{@{}ll@{}}
\toprule
...
\begin{answerBox}
\textbf{问题 9.}\ delta delay 现为\textbf{负}，合理——这是 hold 报告，数据路径上应考虑最坏情况下的串扰\textbf{加速}效应。
\end{answerBox}

\begin{answerBox}
\textbf{问题 10.}\ 违例时序路径上 setup 与 hold 的最大 delta delay 均为 \textbf{0.1738\,ns}：
\begin{lstlisting}
...
\begin{answerBox}
\textbf{问题 11.}\ 这些是 \textbf{victim} 网。
\end{answerBox}

\begin{answerBox}
\textbf{问题 12.}\ 是。默认 \opt{-slack\_lesser\_than} 为 0，因此报告的 victim 网均位于 setup 或 hold 违例路径上。
\end{answerBox}

\begin{answerBox}
\textbf{问题 13.}\ 开关为 \opt{-include\_clock\_nets}：
\begin{lstlisting}
...
\begin{answerBox}
\textbf{问题 14.}\ 使用 \cmd{set_coupling_separation} 增大 victim 与 aggressor 间距，从而减小耦合电容。
\end{answerBox}

\begin{answerBox}
\textbf{问题 15.}\ cost 是 delta delay 与 stage delay 的\textbf{比值}：
\begin{lstlisting}
...
\begin{answerBox}
\textbf{问题 16.}\ 报告的网是 \textbf{victim}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 17.}\ 大比值通常由 victim 转换较慢（弱 victim）引起；可通过\textbf{加大 victim 驱动}或在 victim 上\textbf{插入 buffer} 修复。
\end{answerBox}

\begin{answerBox}
\textbf{问题 18.}\ cost 是\textbf{相对 VDD 的比值}——该 aggressor 网在所有相关违例 victim 网上引起的切换 bump 之和：
\begin{lstlisting}
...
\begin{answerBox}
\textbf{问题 19.}\ 这些是 \textbf{aggressor} 网。
\end{answerBox}

\begin{answerBox}
\textbf{问题 20.}\ \textbf{不一定}。只有 victim 网必须位于违例时序路径上（由 \opt{-slack\_lesser\_than} 定义）。
\end{answerBox}

\begin{answerBox}
\textbf{问题 21.}\ \textbf{缩小}强 aggressor 驱动，或通过网隔离减小耦合电容。
\end{answerBox}

\begin{answerBox}
\textbf{问题 22.}\ \textbf{是}。最坏时钟偏斜未被 delta delay 贡献恶化：
\begin{lstlisting}
