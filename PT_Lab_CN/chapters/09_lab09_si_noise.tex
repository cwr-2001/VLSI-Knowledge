...
\begin{questionBox}
\textbf{问题 1.}\ 列出该脚本中与噪声分析相关的重要步骤（至少三条）。
\end{questionBox}
...
\quad$\square$ Noise propagation \quad$\square$ Timing windows are used to filter aggressors
\end{questionBox}

  \item 打开日志文件 \cmd{noise.log} 并回答：
\begin{questionBox}
\textbf{问题 3.}\ 哪条命令触发了 timing update（查找消息 \textbf{UITE-214}）？\\
\textbf{问题 4.}\ 该 timing update 是否包含串扰延迟分析？\\
\textbf{问题 5.}\ 哪条命令触发了 noise update（查找消息 \textbf{NOISE-010}）？\\
\textbf{问题 6.}\ 根据课堂讲义，timing update 为噪声分析提供哪些信息？
\end{questionBox}
\end{enumerate}
...
\begin{questionBox}
\textbf{问题 7.}\ 本设计是否存在噪声违例？\\
\textbf{问题 8.}\ 最坏噪声 slack（面积单位）是多少？相关区域是 \texttt{above\_low} 还是 \texttt{below\_high}？\\
\textbf{问题 9.}\ 对该“最坏”victim 网列出了多少个“有效（effective）”aggressor 网？\\
\textbf{问题 10.}\ 对该 victim 网，给出驱动单元 pin 名、负载 pin 名与网名。\\
\textbf{问题 11.}\ 解释报告中 ``Propagated'' 的含义，并说明该行为何可能为空（显示为破折号）。
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 12.}\ 解释 ``Steady state resistance source: library set iv curve'' 的含义。\\
\textbf{问题 13.}\ 是否对该网应用了噪声 derating 因子？是否与运行脚本一致？\\
\textbf{问题 14.}\ 解释 ``noise effort threshold'' 的含义。
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 16.}\ 解释 ``Constraint type: library immunity table'' 的含义。\\
\textbf{问题 17.}\ 噪声约束（库电压单位下的所需噪声 bump 高度）是多少？\\
\textbf{问题 18.}\ slack 的 ``area'' 与上一节噪声 bump 的 ``area'' 有何不同？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 19.}\ 最差的触发器数据 pin 距离约束有多远（裕量很大还是很小）？\\
\textbf{问题 20.}\ 解释 slack 为 \texttt{INFINITY} 的含义。
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 21.}\ 有多少条网的噪声 bump 高度大于 0.5？
\end{questionBox}
...
\section{答案 / 解答}
\label{lab9:solutions}

\noindent\textit{Answers / Solutions}

\begin{answerBox}
\textbf{问题 1.}\ 与噪声分析相关的步骤示例（任选其三即可）：
\begin{itemize}
...
\begin{answerBox}
\textbf{问题 3.}\ \cmd{check\_timing} 触发了 timing update。
\end{answerBox}

\begin{answerBox}
\textbf{问题 4.}\ \textbf{是}。可从 \textbf{XTALK-xxx} 消息看出。
\end{answerBox}

\begin{answerBox}
\textbf{问题 5.}\ \cmd{update\_noise} 触发了 noise update。\\
相关消息由应用变量 \cmd{si\_noise\_update\_status\_level} 开启。
\end{answerBox}

\begin{answerBox}
\textbf{问题 6.}\ 使用 aggressor 网的\textbf{到达窗口（arrival windows）}进行滤波；
仅窗口重叠的 aggressor 被定义为 active，并参与噪声 bump 计算。
\end{answerBox}

\begin{answerBox}
\textbf{问题 7.}\ \textbf{没有}噪声违例。
\end{answerBox}

\begin{answerBox}
\textbf{问题 8.}\ 最坏 slack 为 \textbf{153.8006}（\texttt{area} 单位），区域为 \textbf{above\_low}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 9.}\ 有 \textbf{1} 个有效 aggressor 网：\cmd{n25422}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 10.}\ victim 网名 \cmd{n25449}；驱动 pin \cmd{T3\_reg\_190/QN}；负载 pin \cmd{U11405/A1}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 11.}\ ``Propagated'' 行用于\textbf{前级网噪声经驱动单元传播}到本网的噪声。
该行可能为空，因为未开启噪声传播（本分析即如此），和/或库不支持噪声传播（本实验库亦不支持）。
\end{answerBox}

\begin{answerBox}
\textbf{问题 12.}\ 表示计算噪声 bump 高度时，驱动输出稳态 I-V 特性所用的模型；\\
其他选项例如 \cmd{set\_steady\_state\_resistance} 设置的 ``user set value''，
或 PrimeTime 估计线性电阻时的 ``estimation set value''。
\end{answerBox}

\begin{answerBox}
\textbf{问题 13.}\ \textbf{是}。高度与宽度缩放因子均为 \textbf{1.5}，与运行脚本一致。
\end{answerBox}

\begin{answerBox}
\textbf{问题 14.}\ 噪声 bump 高度大于轨到轨电压的 \textbf{0.2} 倍（亦为默认值）的网，
将使用基于 Arnoldi 阶约化的更复杂计算器。
\end{answerBox}

\begin{answerBox}
\textbf{问题 15.}\ \cmd{set\_si\_noise\_analysis}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 16.}\ 表示计算噪声约束（允许/所需噪声 bump 高度）所用的模型；\\
其他选项例如 \cmd{set\_noise\_margin} 的 ``user margin''，或库与用户均未提供约束时的 ``none''。
\end{answerBox}

\begin{answerBox}
\textbf{问题 17.}\ 噪声约束为 \textbf{0.6009}（库电压单位）。
\end{answerBox}

\begin{answerBox}
\textbf{问题 18.}\ 噪声 bump area 为 \textbf{\%height $\times$ width}；\\
...
\begin{answerBox}
\textbf{问题 20.}\ 若计算的噪声 bump 为零，则 slack 定义为 \textbf{INFINITY}。\\
可能原因：该网无有效 aggressor，或所有有效 aggressor 均被筛除。
\end{answerBox}

\begin{answerBox}
\textbf{问题 21.}\ 在 \texttt{below\_high} 区域，有 \textbf{4} 条网的噪声 bump 高度大于 0.5：
\begin{lstlisting}
pt_shell> report_noise_bumps -bump_greater_than 0.5
Pin Name    Net Name
U11361/A1   n25502
