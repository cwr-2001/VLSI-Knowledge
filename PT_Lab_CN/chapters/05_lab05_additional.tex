...
\begin{questionBox}
\textbf{问题 1.}\ 你用时序报告中哪些行来确认这是 setup 报告，且路径起点为时钟 \cmd{SDRAM\_CLK} 的 source？\\[0.35em]
\textbf{答：}\\
\begin{lstlisting}
...
\textbf{问题 2.}\ 该时序报告如何确认警告中的 pin 位于 data path（即时钟 source 被当作数据路径使用并约束），而非 clock path？\\[0.35em]
\textbf{答：} 若未生成报告（``path is unconstrained''），则该 pin 位于 clock path。
由于生成了时序报告，该 pin 位于 \textbf{data path}。

\textbf{问题 3.}\ 经过上述 pin 传播的是哪种 sense（positive unate 还是 negative unate）？
在时序报告中查找标示该 pin 的小箭头。\\[0.35em]
\textbf{答：} 经过该 pin 的是 \textbf{positive unate} 时序弧（fall to fall）。
数据通路中的相关行：
\begin{lstlisting}
...
且该 pin 的时序弧为 negative unate？\\[0.35em]
\textbf{答：}\\
\begin{lstlisting}
...
\textbf{问题 5.}\ 说明为何对这些时序路径可忽略（并抑制）该警告。\\[0.35em]
\textbf{答：} 该消息表示传播时钟经过该 MUX 时将同时使用反相与非反相 sense——这是默认行为。
...
\begin{questionBox}
\textbf{问题 6.}\ 该 latch 的 clock pin 名称是什么？\\[0.35em]
\textbf{答：}\\
\begin{lstlisting}
...
\textbf{问题 7.}\ 三个 data pin 的名称分别是什么？\\[0.35em]
\textbf{答：}\\
\begin{lstlisting}
pt_shell> all_registers -level_sensitive -data_pins
...
\begin{questionBox}
\textbf{问题 8.}\ 你如何知道该 latch 未从上一级经历 time borrow？\\[0.35em]
\textbf{答：} 若该 latch 正在经历 time borrow，报告中会出现一行说明给予起点（即上一级）的时间。
...
\begin{questionBox}
\textbf{问题 9.}\ path segment \#1 还能再花多少时间才会开始从 path segment \#2 借用时间？\\[0.35em]
\textbf{答：} 还可再花 \textbf{3.465\,ns} 才会开始从 path segment \#2 借用时间
（等于正 slack）。
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 10.}\ path segment \#1 从 path segment \#2 借用了多少时间？\\[0.35em]
\textbf{答：} path segment \#1 借用了 \textbf{0.5306\,ns}（在时序报告的 data required time 段中注明）。

\textbf{问题 11.}\ path segment \#1 的 slack 是多少？\\[0.35em]
\textbf{答：} slack 为 \textbf{0}。
...
\begin{questionBox}
\textbf{问题 12.}\ 给予 path segment \#1 的时间是否符合你的预期？\\[0.35em]
\textbf{答：} 是——path segment \#2 时序报告中给予起点的时间，
...
\begin{questionBox}
\textbf{问题 13.}\ 起点（startpoint）是什么？\\[0.35em]
\textbf{答：} \textbf{I\_ORCA\_TOP/I\_PARSER/blender\_clk\_en\_reg/CP}。
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 14.}\ 路径终点是什么？\\[0.35em]
\textbf{答：} \textbf{I\_ORCA\_TOP/I\_BLENDER/U794/A}（gating check 终点）。

\textbf{问题 15.}\ transparency 的 open 与 close 边分别是多少？\\[0.35em]
\textbf{答：} transparency open 与 close 边分别为 \textbf{6.6454} 与 \textbf{10.4433}。

\textbf{问题 16.}\ 数据是否在两者之间到达？\\[0.35em]
\textbf{答：} 是——数据在 \textbf{7.1759} 到达，介于 open 与 close 边之间。

\textbf{问题 17.}\ 是否发生了 time borrowing？\\[0.35em]
\textbf{答：} \textbf{否}，未发生 time borrowing。

\textbf{问题 18.}\ slack 是否为正？\\[0.35em]
\textbf{答：} \textbf{是}，slack 为正（\textbf{2.1398\,ns}）。
