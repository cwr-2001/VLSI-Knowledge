...
\begin{questionBox}
\textbf{问题 1.}\ 有多少个 setup 检查处于 violated 状态？\\[0.35em]
\textbf{答：} 共有 \textbf{23} 个 setup 检查 violated。

\textbf{问题 2.}\ 最坏违例 slack 的大小是多少？对应的端点（endpoint）是什么？\\[0.35em]
\textbf{答：} 最坏违例 slack 为 \textbf{-0.7737}，端点为\\
...
\begin{questionBox}
\textbf{问题 3.}\ \cmd{report_timing} 如何表明该路径已被重算？\\[0.35em]
\textbf{答：} \textbf{Path Type} 行后出现 \texttt{(recalculated)}。例如：
\begin{lstlisting}
...
\textbf{问题 4.}\ 交互式 path-based analysis（\opt{-pba\_mode path}）之后，最坏 slack 违例是多少？\\[0.35em]
\textbf{答：} 交互式 path-based analysis 之后最坏 slack 为 \textbf{-0.7711}。

\textbf{问题 5.}\ 与 GBA slack 相比，PBA slack 是改善、变差还是不变？结果是否符合预期？\\[0.35em]
\textbf{答：} slack \textbf{有所改善}，符合预期。\\
...
\begin{questionBox}
\textbf{问题 6.}\ GBA 与 PBA 各返回多少条违例路径？\\[0.35em]
\textbf{答：} GBA：\textbf{9} 条违例路径；PBA exhaustive：\textbf{8} 条违例路径。
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 7.}\ exhaustive PBA 之后最坏违例的值是多少？对应的时序路径端点名是什么？\\[0.35em]
\textbf{答：} exhaustive PBA 之前最坏违例为 \textbf{-0.7737}，之后为 \textbf{-0.7711}；\\
端点均为 \cmd{I_ORCA_TOP/I_BLENDER/s4_op2_reg[31]/D}。

\textbf{问题 8.}\ 是否出现 \textbf{UITE-480} 警告？\\[0.35em]
\textbf{答：} 没有 \textbf{UITE-480} 警告——exhaustive PBA 重算已完成。
\end{questionBox}
\end{enumerate}
