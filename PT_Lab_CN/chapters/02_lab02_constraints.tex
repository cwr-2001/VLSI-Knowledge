...
\begin{questionBox}
\textbf{问题 1.}\ 设计中所有寄存器是否都已有时钟？\\[0.35em]
\textbf{答：} 是。在 \cmd{Information: Checking 'no_clock'.} 消息之后，没有报告任何时钟 pin。

\textbf{问题 2.}\ 是否存在缺失的约束？你能解释原因吗？\\[0.35em]
\textbf{答：} 是。有 2 个 output 端口报告缺失 output delay：
\begin{lstlisting}
...
\begin{questionBox}
\textbf{问题 3.}\ 近三分之二的 setup/hold 检查处于 untested 状态！造成这一现象的两个原因是什么？\\[0.35em]
\textbf{答：} \textbf{constant\_disabled} 与 \textbf{false\_path}。
（可使用 \cmd{report\_analysis\_coverage -status untested -check \{setup hold\}} 确认。）

\textbf{问题 4.}\ 为何存在未行使（unexercised）的 \cmd{min_pulse_width} 检查？\\[0.35em]
\textbf{答：} \cmd{min\_pulse\_width} 检查仅当 pin 上有时钟时才会被行使。
由于这些是诸如 set/clear 之类的非时钟异步 pin，未在其上定义时钟。
（可使用 \cmd{report\_analysis\_coverage -status untested -check min\_pulse\_width} 确认。）

\textbf{问题 5.}\ setup 与 hold 各有多少个 output delay 约束？这些约束是满足（met）还是违例（violated）？\\[0.35em]
\textbf{答：} 从 \cmd{report\_analysis\_coverage} 可见：setup 与 hold 各有 \textbf{75} 个 output delay 约束；
...
\begin{questionBox}
\textbf{问题 6.}\ 到达 \cmd{pad[0]} 的最小与最大到达时间（arrival time）分别是多少？\\[0.35em]
\textbf{答：} 最小与最大到达时间分别为 \textbf{2\,ns} 与 \textbf{8\,ns}
（rise 与 fall 数据跳变在该端口使用相同约束值）。

\textbf{问题 7.}\ 约束 \cmd{pad[0]} 的外部起点时钟名称是什么？\\[0.35em]
\textbf{答：} 时钟名称为 \textbf{PCI\_CLK}。
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 8.}\ 你使用了时序报告中的哪些行来确认该路径起始于端口 \cmd{pad[0]}，且为 setup 报告？\\[0.35em]
\textbf{答：}\\
\begin{lstlisting}
pt_shell> report_timing -from pad[0]
...
\textbf{问题 9.}\ 列出该时序报告中涉及的全部用户指定约束。\\[0.35em]
\textbf{答：} 时钟周期是一项约束；时钟 \cmd{PCI\_CLK} 为 propagated（非 ideal）；
输入外部延迟（来自 input delay 约束）也是用户指定约束。

\textbf{问题 10.}\ 对于起点时钟 \cmd{PCI_CLK}，时钟延迟（clock latency）必须包含在何处？\\[0.35em]
\textbf{答：} 时钟网络延迟为零，因此表示外部时钟延迟的唯一位置是 input delay 约束中的输入外部延迟（input external delay）。
正确建模方式是：对 \cmd{set\_input\_delay} 使用 \opt{-network\_latency\_included} 与 \opt{-source\_latency\_included}。

\textbf{问题 11.}\ 描述端口 \cmd{pad[0]} 的方向（即它是 input、output 还是 inout 端口）。\\[0.35em]
\textbf{答：} 端口 \cmd{pad[0]} 是 \textbf{inout} 端口，因此它既是时序路径起点，也是时序路径终点。
\begin{lstlisting}
Point                                    Incr      Path
...
\textbf{问题 12.}\ 描述该时序路径的终点（即它是 output 端口还是内部触发器）。\\[0.35em]
\textbf{答：} 终点是由 \cmd{PCI\_CLK} 时钟的上升沿触发 flip-flop
...
\begin{questionBox}
\textbf{问题 13.}\ 对于终点时钟 \cmd{PCI_CLK}，时钟源延迟（clock source latency）与时钟网络延迟各有多大？\\[0.35em]
\textbf{答：} 下方仅展示时序报告的 data required time 段。
...
\textbf{问题 14.}\ 时钟 \cmd{PCI_CLK} 在何处定义（时钟定义点）？\\[0.35em]
\textbf{答：} 时钟 \cmd{PCI\_CLK} 在输入端口 \cmd{pclk} 处定义。
...
\begin{questionBox}
\textbf{问题 15.}\ 输入外部延迟（input external delay）约束的值是否符合你的预期？\\[0.35em]
\textbf{答：} 是。由上文 \cmd{report\_port} 可知，输入外部延迟相对于 \cmd{PCI\_CLK} 上升沿应为 \textbf{2\,ns}，
时序报告确认了这一点：
\begin{lstlisting}
pt_shell> report_timing -delay min -from pad[0]
Startpoint: pad[0] (input port clocked by PCI_CLK)
...
\begin{questionBox}
\textbf{问题 16.}\ 该端口的最小与最大输出延迟约束分别是多少？\\[0.35em]
\textbf{答：}\\
\begin{lstlisting}
pt_shell> report_port -output_delay pad[0]
Output Delay
...
\textbf{问题 17.}\ 负的最小输出延迟约束将如何施加于该端口（即它会施加正向还是负向的 hold 要求）？\\[0.35em]
\textbf{答：} 课堂讲义指出：负的 hold output delay 约束会施加\textbf{正向} hold 要求。

\textbf{问题 18.}\ 约束该端口的外部终点时钟名称是什么？\\[0.35em]
\textbf{答：} 端口 \cmd{pad[0]} 相对于 \textbf{PCI\_CLK} 施加约束。
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 19.}\ 描述该时序路径的起点（即它是 input 端口还是内部触发器）。\\[0.35em]
\textbf{答：} 时序路径起点是内部 flip-flop：
\begin{lstlisting}
...
\textbf{问题 20.}\ 该时序路径的 path group 是否符合你的预期？\\[0.35em]
\textbf{答：} 是。path group 为 \textbf{PCI\_CLK}，与外部捕获时钟名称一致。

\textbf{问题 21.}\ ``data required time'' 是否符合你的预期？\\[0.35em]
\textbf{答：} 是。捕获时钟边为 0\,ns，这对 hold 是合适的。
