...
\begin{questionBox}
\textbf{问题 1.}\ 该变量的默认值是多少？\textbf{［提示：man page］}
\end{questionBox}
\end{enumerate}
...
\begin{questionBox}
\textbf{问题 2.}\ 找出 slack 最差的 5 个 setup 违例，需给出端点名称与 slack。\\
\textbf{问题 3.}\ 列出存在违例 setup 时序路径的 2 个时钟域，以及存在违例 hold 时序路径的 5 个时钟域
（ORCA 共有 6 个时钟域）。\\
\textbf{问题 4.}\ 分别有多少 hold 违例位于 input 路径、output 路径，以及 register-to-register 路径？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 5.}\ 列出 slack 裕量最大（最好 slack）的端点。
\end{questionBox}

  \item 生成设计质量的高层概览：

\begin{questionBox}
\textbf{问题 6.}\ 哪个 clock group 的违例路径数量最多？
\end{questionBox}
\end{enumerate}
...
\begin{questionBox}
\textbf{问题 7.}\ 该时序路径是满足（meet）还是违例（violate）时序？\\
\textbf{问题 8.}\ 该时序路径属于哪一类——内部 flip-flop 到 flip-flop、input，还是 output 时序路径？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 9.}\ 该时序路径属于哪一类——内部 flip-flop 到 flip-flop、input，还是 output 时序路径？\\
\textbf{问题 10.}\ 该时序路径的数据通路上有多少个单元（cell）？\\
\textbf{问题 11.}\ 起点 flip-flop 的 CP 到 Q pin 使用的是上升沿（rise）单元延迟。
        请给出一个可能的原因：为何这比使用该 flip-flop 更快的下降沿（fall）延迟会导致更差的 hold slack？
\end{questionBox}

  下一步将继续探索并确认你对上述问题的回答。

\begin{questionBox}
\textbf{问题 12.}\ 要确认上述问题的答案，你还需要哪些额外信息？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 13.}\ 你使用了该报告中的哪些行来确认报告了正确的路径？\\
\textbf{问题 14.}\ 猜测是否正确——更快的 fall 延迟导致更快的数据到达时间，
        但更小的 hold 时间要求，从而得到更好的 slack？
\end{questionBox}
\end{enumerate}
...
\begin{questionBox}
\textbf{问题 15.}\ 写出为每个 path group 生成一条 setup 时序报告的命令。\\
\textbf{问题 16.}\ 写出为每个存在违例的 path group 生成一条 setup 时序报告的命令。\\
\textbf{问题 17.}\ ORCA 中存在违例时序路径的两个 path group 名称是什么（答案来自上一题的结果）？\\
\textbf{问题 18.}\ 写出为受 \cmd{PCI_CLK} 约束的任意 output 端口生成 setup 最差 slack 时序报告的命令。\\
\textbf{问题 19.}\ ORCA 中有若干 latch；写出识别这些 latch 数据 pin 的命令。\\
\textbf{问题 20.}\ 写出为 \cmd{latched_clk_en_reg} latch 的 D pin 生成 hold 时序报告的命令。
\end{questionBox}
\end{enumerate}
...
描述 setup 时序报告中将使用的具体时钟边。\\
\textbf{问题 23.}\ 对于同一条时序路径，描述 hold 时序检查将使用的具体时钟边。
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 24.}\ 哪个开关可用于为每类半时钟周期时序路径生成最差的 10 条时序报告？\\
\textbf{问题 25.}\ 为何 PrimeTime 报告 ``no constrained paths''？
（提示——PrimeTime 使用的选项显示在 \cmd{report_timing} 命令之后。）\\
\textbf{问题 26.}\ 要报告最差的 10 条时序路径，还必须使用哪个额外选项？
\end{questionBox}
...
\begin{answerBox}
\textbf{问题 2.}\ 下列命令将列出按 slack 排序的全部 setup 违例。
...
\begin{answerBox}
\textbf{问题 5.}\ output 端口 \cmd{sd_DQ[0]} 的 slack 最好，为 \textbf{1.5994\,ns}。
...
\begin{answerBox}
\textbf{问题 7.}\ 违例时序，slack 为 \textbf{$-1.157$\,ns}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 8.}\ 这是一条 \textbf{output} 时序路径，终点为 output 端口 \cmd{pad[1]}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 9.}\ 这是一条 \textbf{内部}时序路径。终点外观类似 flip-flop，
实为 RAM 时序模型中的众多时序弧之一。
\end{answerBox}

\begin{answerBox}
\textbf{问题 10.}\ 数据通路上仅有 \textbf{1} 个单元——起点 flip-flop。
该时序路径由起点 flip-flop 直接连到终点 flip-flop。
\end{answerBox}

\begin{answerBox}
\textbf{问题 11.}\ 通常 fall 延迟比 rise 延迟更快，会对 hold 产生更差的 slack！
本例的例外是：若终点 flip-flop 数据 pin 的 fall 跳变导致更小的 library hold time 要求——这正是本例的情况。
\end{answerBox}

\begin{answerBox}
\textbf{问题 12.}\ 再生成一份以终点下降沿跳变计算数据到达时间的时序报告，并比较两份报告。
...
\begin{answerBox}
\textbf{问题 14.}\ 是！数据到达时间更快（\textbf{1.139\,ns} 对 \textbf{1.150\,ns}），
...
\begin{answerBox}
\textbf{问题 17.}\ 两个 path group 为 \textbf{PCI\_CLK} 与 \textbf{SYS\_CLK}。
\end{answerBox}
...
\begin{answerBox}
\textbf{问题 21.}\ \cmd{SDRAM\_CLK} 上升沿在 \textbf{0\,ns}，下降沿在 \textbf{3.75\,ns}，周期为 \textbf{7.50\,ns}。
占空比为 \textbf{50\%}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 22.}\ 对于由 \cmd{SDRAM\_CLK} 上升沿约束到下降沿的时序路径，setup 使用的时钟边为 \textbf{0\,ns} 到 \textbf{3.75\,ns}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 23.}\ 对于同一条时序路径，hold 使用的时钟边为 \textbf{7.5\,ns} 到 \textbf{3.75\,ns}。
\begin{lstlisting}
...
\begin{answerBox}
\textbf{问题 24.}\ 在上述命令中添加开关 \opt{-max\_paths 10}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 25.}\ \opt{-max\_paths} 会隐式设置另一选项 \opt{slack\_lesser\_than 0}；
由于 slack 为正值，因此没有可报告的路径。
\end{answerBox}

\begin{answerBox}
\textbf{问题 26.}\ 由于不存在违例路径可报告，须添加选项：\opt{slack\_lesser\_than 100}。
\end{answerBox}
