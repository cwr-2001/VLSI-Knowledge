...
\begin{questionBox}
\textbf{问题 1.}\ 设计中有多少个时钟？其中有多少个是 generated clock？\\
\textbf{问题 2.}\ 哪些输入端口定义了 master clock？\\
\textbf{问题 3.}\ 哪些输出端口定义了 outgoing clock？\\
\textbf{问题 4.}\ 时钟是 propagated 还是 ideal？\\
\textbf{问题 5.}\ 哪 3 对时钟之间存在已约束的时序路径？
\end{questionBox}
\end{enumerate}
...
\begin{questionBox}
\textbf{问题 6.}\ \cmd{SYS\_2x\_CLK} 的 master clock 是什么？\\
\textbf{问题 7.}\ \cmd{SYS\_2x\_CLK} 定义在哪个 pin/port（其「source」）上？
（提示：可能需要拖开时钟矩阵窗口以露出更多时钟信息列。）\\
\textbf{问题 8.}\ \cmd{SYS\_2x\_CLK} 的 master clock 定义在哪个 pin/port 上？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 9.}\ 哪个端口连接到 MUX \cmd{I\_CLOCK\_GEN/U20} 的 select pin？\\
\textbf{问题 10.}\ 查看原理图是否有助于理解测试模式下的时钟方案？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 11.}\ 哪些时钟驱动 \cmd{I\_CLOCK\_GEN/U10}？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 12.}\ 哪个端口驱动 \cmd{I\_CLOCK\_GEN/U10} 的 select 线？
\end{questionBox}
...
\cmd{SYS\_2x\_CLK} 时钟原理图？（在 \cmd{SYS\_2x\_CLK} 上右键查找相应选项。）
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 14.}\ capture delay 中有百分之多少来自 CRP？\\
\textbf{问题 15.}\ 该百分比是否对所有设计都具有代表性？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 16.}\ 该 buffer 输出 pin 的 arrival window 有多宽？\\
\textbf{问题 17.}\ 这是否与先前在 Path Inspector 的 data arrival / data required 段中看到的一致？
\end{questionBox}

  \item 检查路径波形：点击 Path Inspector 窗口底部的 \textbf{Waveform} 标签。

\begin{questionBox}
\textbf{问题 18.}\ 在波形窗口中右键点击可添加什么内容？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 19.}\ ORCA 有多少个违例？分别是什么类型？
\end{questionBox}
...
（可借助 Job Aid「Timing Reports」回忆两个合适的开关。）\\
\textbf{问题 21.}\ 该违例涉及哪些时钟（launch 与 capture）？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 22.}\ 为何 PrimeTime 未为 outgoing clock \cmd{SD\_DDR\_CLK} 计算 source latency？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 23.}\ 该变量的名称是什么？\\
\textbf{问题 24.}\ 根据 man page，该变量将做什么？
\end{questionBox}
...
\begin{questionBox}
\textbf{问题 25.}\ 如何修改 \cmd{check\_timing} 以添加检查，验证所有时钟均为 propagated？
\end{questionBox}
...
\section{答案 / 解答}
\label{lab4:solutions}

\noindent\textit{Answers / Solutions}

\begin{answerBox}
\textbf{问题 1.}\ 可用 \cmd{report\_clock} 或下列命令获取：
\begin{lstlisting}
...
\begin{answerBox}
\textbf{问题 3.}\ 由同一报告可知，输出端口为 \textbf{sd\_CK} 与 \textbf{sd\_CKn}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 4.}\ 用 \cmd{report\_clock} 可见，设计中所有时钟均为 \textbf{ideal}。
\end{answerBox}
...
\begin{answerBox}
\textbf{问题 6.}\ \textbf{SYS\_CLK}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 7.}\ \textbf{I\_CLOCK\_GEN/I\_CLKMUL/CLK\_2x}。
（可能需要拖开矩阵窗口才能看到 source pin。）
\end{answerBox}

\begin{answerBox}
\textbf{问题 8.}\ \textbf{sys\_clk}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 9.}\ \textbf{test\_mode}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 10.}\ 是——两个时钟进入 MUX：一个来自时钟发生器，一个直接来自端口。
\cmd{test\_mode} 端口控制 select 线，使测试模式下可旁路时钟发生器，
让设计直接由端口驱动。
\end{answerBox}

\begin{answerBox}
\textbf{问题 11.}\ \textbf{SYS\_CLK} 与 \textbf{SYS\_2x\_CLK}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 12.}\ \textbf{power\_save}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 13.}\ 可以。选中 \cmd{SYS\_2x\_CLK}（可能需要放大以仅选中该时钟），
右键选择 \textbf{Schematic for Selected Clocks}。
可能需要双击展开输入或输出 stub 才能得到与 Clock Analyzer 中完全相同的原理图。
\end{answerBox}

\begin{answerBox}
\textbf{问题 14.}\ \textbf{4.18\%}。
\end{answerBox}

\begin{answerBox}
\textbf{问题 15.}\ 否——该数值取决于具体设计与具体路径。
\end{answerBox}

\begin{answerBox}
\textbf{问题 16.}\ 为 $2.86939 - 2.40107 \approx \mathbf{0.468}$\,ns。
\end{answerBox}
...
\begin{answerBox}
\textbf{问题 18.}\ 可添加输入 pin 与输出 pin，从而查看整条路径的细节。
\end{answerBox}

\begin{answerBox}
\textbf{问题 19.}\ 共有 \textbf{23} 个 setup 违例、\textbf{53} 个 hold 违例、\textbf{32} 个 out\_setup 违例。
\end{answerBox}
...
\begin{answerBox}
\textbf{问题 22.}\ 必须为 propagated clock（尤其是 master clock），
PrimeTime 才能为 generated clock 计算 source latency。
本设计中所有时钟均为 ideal。
\end{answerBox}

\begin{answerBox}
\textbf{问题 23.}\ \textbf{timing\_all\_clocks\_propagated}。
\begin{lstlisting}
pt_shell> aa propagate
...
...
\begin{answerBox}
\textbf{问题 24.}\ 将该变量设为 \texttt{true} 后，之后创建的所有时钟都将以 propagated clock 方式创建。
（下一任务中将实际设置该变量。）
\end{answerBox}

\begin{answerBox}
\textbf{问题 25.}\ 附加检查名为 \textbf{ideal\_clocks}。
